#!/usr/bin/env python3
"""Verify the fixed SurfaceMesh_from_stl produces correct normals."""
import sys
...
def main():
    for ship_name, stl_file in [("KVLCC2", "KVLCC2_Hull.stl"), ("DTMB5415", "DTMB5415_Hull.stl")]:
        ...
        # Build the mesh using the FIXED SurfaceMesh_from_stl
        ...
        print(f"\n  Bow cells (ix < {hull_center_x_lattice - 5:.0f}): {bow_mask.sum()}")
        if bow_mask.any():
        ...
        print(f"  Stern cells (ix > {hull_center_x_lattice + 5:.0f}): {stern_mask.sum()}")
        if stern_mask.any():
        ...
        # Check x-component of pressure drag proxy
        # F_px = -sum(p * nx * dA) — if normals are correct, this should be positive
        # (pressure on bow pushes the ship forward = +x, but drag is the force
        # on the fluid, which is -x... actually, the formula is F = -sum(p*n*dA)
        # which is the force on the wall. For drag, we want Fx > 0.
        # With uniform p=1 (test), Fx = -sum(nx*dA)
        sum_nx_dA_uniform = (mesh_normals[:, 0]).sum()
        print(f"\n  Pressure drag proxy (uniform p=1):")
        print(f"    -sum(nx): {-sum_nx_dA_uniform:.2f} (should be >0 for positive drag)")
