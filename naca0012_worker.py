"""NACA 0012 airfoil drag validation worker — D3Q19 MRT+Smag + wall_function + far_field.

Usage:
    PYTHONPATH=src python naca0012_worker.py <device_id> <angle_deg> <Cs>
...
def wallfn(f: torch.Tensor, solid: torch.Tensor, nu: float, y_val: float = 0.5):
    """Apply log-law wall function body force + return friction/pressure drag.

    Returns (f_corrected, friction_force_x, pressure_force_x).
    """
    device = f.device
...
def main():
    ...
    ref_area = chord * float(ny)  # 80 * 8 = 640
