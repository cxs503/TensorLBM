#!/usr/bin/env python3
"""SUBOFF bare-hull time-convergence study — Re=1000, MRT+Smagorinsky.

Runs MRT+Smagorinsky (Cs=0.05) on SUBOFF bare hull with different step counts
to check if Cd converges with more simulation time.

Parameters (fixed across all runs):
  L=80, nx=200, ny=80, nz=80, u_in=0.06, Re=1000, tau=0.5144
  dpS = 0.5*u_in^2 * pi * D * L   (WETTED area, D=2*R_max=9.328)
  Reference: Cf = 1.328/sqrt(Re) = 0.042  (Blasius laminar flat plate)
  MRT+Smagorinsky (Cs=0.05), from_gradient normals
  Averaging window = 500 (last 500 steps)

Usage:
  python suboff_time_conv_worker.py <n_steps> <device_id> <output_path>
  n_steps: 5000 | 10000 | 20000
"""
import json
import math
import sys
...
def run(n_steps, device_id, output_path=None):
    tag = f"[SDAA:{device_id} SUBOFF Re=1000 t={n_steps}]"
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python suboff_time_conv_worker.py <n_steps> <device_id> [output_path]")
        print("  n_steps: 5000 | 10000 | 20000")
        sys.exit(1)
    ...
    run(n_steps, device_id, output_path)
