#!/usr/bin/env python3
"""SUBOFF pressure-extrapolation comparison.

Three runs on SUBOFF bare hull Re=1000, L=80, nx=200, ny=80, nz=80 with
identical solver settings; only the wall-pressure extrapolation differs:

  none      → p_wall = p1                  (current: pressure at near-wall cell)
  linear    → p_wall = 2·p1 - p2           (1st-order extrapolation to wall)
  quadratic → p_wall = 3·p1 - 3·p2 + p3    (2nd-order extrapolation to wall)

The 2nd/3rd cells are found by shifting the pressure field 1/2 lattice steps
along the dominant normal direction (see ``_shift_along_normal_dominant``).

Usage:
  python suboff_pressure_extrap_worker.py <extrap_method> <device_id> <output_path>
  extrap_method: none | linear | quadratic
"""
import json
import math
import sys
...
def run(device_id, extrap, Re, L, nx, ny, nz, u_in, tau, n_steps,
    ...
    print(
        ...
        f"dpS={dpS:.6e} Cf_ref={Cf_ref} extrap={extrap}",
        flush=True,
    )
    ...
    # Build surface mesh (analytical axisymmetric normal)
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python suboff_pressure_extrap_worker.py <extrap_method> <device_id> <output_path>")
        print("  extrap_method: none | linear | quadratic")
        sys.exit(1)
    ...
    run(
        ...
        n_steps=5000,
        Cf_ref=0.042,
        tag=f"[SDAA:{device_id} SUBOFF Re=1000 extrap={extrap}]",
