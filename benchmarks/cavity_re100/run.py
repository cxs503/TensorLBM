#!/usr/bin/env python3
"""方腔流（lid-driven cavity）Re=100 benchmark — Ghia (1982) 验证.
...
物理设置
  ...
  - Re = U0·H/ν = 100，H=nx，ν=(τ−0.5)/3 ⇒ τ = 3·U0·nx/Re + 0.5
  - 网格: 128×128 与 192×192（≥2 档网格证明收敛）
  - 稳态: ≥100000 步或残差<1e-8；每 5000 步记录残差（内部格点 max|Δu|/步）

对比 Ghia et al. (1982) 表值（库内 GHIA_RE100 数据，129×129 多网格解）：
  - 中线 u(x=0.5, y) 与 v(x, y=0.5) 全剖面（17 点 × 2 条中线）
  - 关键点: u@y=0.25/0.5/0.75、v@x=0.25/0.5/0.75
  - 主涡涡心 (0.6172, 0.7344)

判定: 中线速度对 Ghia 最大偏差 ≤3% 且 128/192 收敛 → 写入 benchmarks/verified/cavity_re100/。
用法: PYTHONPATH=src python benchmarks/cavity_re100/run.py [--grids 128,192] [--max-steps 100000]
"""
...
GHIA_VORTEX = (0.6172, 0.7344)  # Ghia (1982) Re=100 主涡涡心
KEY_POINTS = {
...
def run_grid(
    ...
    # 关键点（u@y=0.25/0.5/0.75，v@x=0.25/0.5/0.75）
    # 注意：GHIA_RE100["y"] 是降序（1.0→0.0），np.interp 需要升序 xp
...
def main() -> None:
    ...
    summary = {
        "case": "cavity_re100",
        "description": "方腔流 Re=100，D2Q9 MRT，Ghia(1982) 验证",
        "u_lid": U_LID,
        "re": RE,
        "collision": "mrt",
        "lattice": "D2Q9",
