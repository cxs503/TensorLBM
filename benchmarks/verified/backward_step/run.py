...
实现说明（真实模拟, 无外推）:
- run_backward_facing_step 为共性模块入口, 本脚本以 monkey-patch 注入, 不修改库文件
  (缺口记录见 /tmp/backward_gap.md):
  1) bfs._apply_bfs_inlet  -> 抛物线入口 (库内硬编码均匀入口)
  2) bfs._apply_bfs_outlet -> Zou/He 压力出口 (库内零梯度 copy 出口质量漂移无界)
  3) bfs.measure_reattachment_length -> 亚格插值测量 + 捕获速度场快照
- A/B 诊断开关 (环境变量): BFS_FORCE_BGK=1 (BGK 对照 MRT),
  BFS_UNIFORM_INLET=1 (均匀入口对照抛物线, 需配合 BFS_U_MAX=U_avg, BFS_RE=U_avg*h/nu 保持通量一致)
- 输出: run_dir/run_metadata.json (模块), result.json (本脚本, 路径见 BFS_VERIFIED_DIR)
"""
import json
import os
import sys
...
def main() -> None:
    ...
    config = BackwardFacingStepConfig(
        ...
        run_name=f"armaly_re{RE:g}_er{ER:.3f}_h{STEP_H}",
        seed=0,
        device=DEVICE,
        overwrite=True,
        use_compile=False,
    ...
    result = {
        ...
        "geometry": {
            ...
            if os.environ.get("BFS_UNIFORM_INLET", "0") != "1"
            else "uniform (A/B, flux-matched: U_in=U_avg=%.4f)" % U_MAX,
        },
        "physics": {
            "re": RE, "nu": NU, "tau": TAU,
            "collision": "BGK (forced)" if os.environ.get("BFS_FORCE_BGK", "0") == "1"
            else "MRT (module auto, tau<0.60)",
        },
        "result": {
            "xr_h": xr_h,
            "xr_h_module_convention": xr_h_mod,
            "err_pct_vs_3.0": err_pct,
            "xr_series": xr_series,
            "steps": steps_arr,
