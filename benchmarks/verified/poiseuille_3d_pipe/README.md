...
**名义解析直接通道（2026-09-19 严格标准复核，控制器亲算）**：不做任何反演/参考
重构，直接观测量——域内中心线速度 $u_{\text{center}}$ 对名义解析 $U_{\max}=2u_{\text{in}}$
的误差为 **R20 −1.12% → R40 −0.81%**（单调，见 result.json `u_max_err_pct`，
run.py `(u_center − U_max_ref)/U_max_ref`）。即在最严格"名义半径直接比"口径下本
案例裸过 3% 门限，$R_{\text{eff}}^Q$ 帧是精度增强而非达标拐杖。
