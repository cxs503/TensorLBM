...
**名义解析直接通道（2026-09-19 严格标准复核，控制器亲算）**：不做 R_eff 反演，
逐 bin 剖面对**名义半径**解析解直接比较：max **2.133% → 0.974% → 0.435%**
（L2 1.53/0.66/0.34%），单调且全 ≤3%——最严格"名义解析直接比"口径下裸过，
R_eff 帧为精度增强而非达标拐杖（与 result.json `nominal_*` 字段逐位一致）。
