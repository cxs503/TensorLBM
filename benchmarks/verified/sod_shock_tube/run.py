...
RESULT = {"case": "sod_shock_tube_isothermal_d2q9", "verified": False,
...
def ana_profile(x, t, rhoL, rhoR, rs, us, W, xd, cs=CS):
    s = (x - xd) / max(t, 1e-12)
    rho = np.empty_like(x); u = np.empty_like(x)
    m1 = s < -cs; m2 = (s >= -cs) & (s < us - cs)
    m3 = (s >= us - cs) & (s < W); m4 = s >= W
    rho[m1] = rhoL; u[m1] = 0.0
    rho[m2] = rhoL * np.exp(-(s[m2] + cs) / cs); u[m2] = s[m2] + cs
    rho[m3] = rs; u[m3] = us
    rho[m4] = rhoR; u[m4] = 0.0
    return rho, u
...
def metrics(profs, rhoL, rhoR, rs, us, W, xd, nx, tlist):
    ...
    for t in tlist:
        ...
        mask = np.zeros(nx, bool); mask[max(lo, 0):min(hi, nx)] = True
        jump = rhoL - rhoR
        ...
        g = np.abs(np.gradient(rho)); g[:wlo] = 0.0; g[whi:] = 0.0
        xsh = int(np.argmax(g))
        mask2 = mask.copy()
        ...
        out[t] = dict(L2_rho=L2r, L2_u=L2u, L2_rho_smooth=L2r_smooth, shock_x=xsh,
                      osc_frac=osc, rho_mid=rho_mid, u_mid=u_mid)
    ts = sorted(out.keys())
...
def main():
    sod_cases = []
    for nx, steps, snaps in [(2000, 400, [100, 200, 300, 400]),
                             (4000, 800, [200, 400, 600, 800])]:
        rhoL, rhoR = 1.0, 0.25
        ...
        sod_cases.append(dict(nx=nx, steps=steps, first_bad_step=bad,
                              rho_star=rs, u_star=us, Ma_mid=us / CS, per_t=per_t,
                              **{k: v for k, v in m.items() if not isinstance(k, int)}))
        print(f"nx={nx}: bad={bad} L2r={m['L2_rho_final']:.5f} "
        ...
        np.savez_compressed(os.path.join(HERE, f"profiles_nx{nx}.npz"),
                            **{f"t{t}": profs[t][0] for t in snaps},
                            **{f"u{t}": profs[t][1] for t in snaps})

    # ---- acoustic supplementary (weak-compressible validation) ----
    acou = []
    for eps, steps, snaps in [(0.01, 400, [100, 200, 300, 400]),
                              (0.25, 800, [200, 400, 600, 800])]:
        nx = 4000
        rhoL, rhoR = 1.0 + eps, 1.0 - eps
        profs, bad = run(nx, 4, rhoL, rhoR, 0.8, steps, snaps)
        rs, us, W = iso_riemann(rhoL, rhoR)
        m = metrics(profs, rhoL, rhoR, rs, us, W, nx // 2, nx, snaps)
        acou.append(dict(eps=eps, first_bad_step=bad, Ma_mid=us / CS,
                         W_fit=m["W_fit"], W_ana=W, W_rel=m["W_rel"],
                         u_mid_rel=m["u_mid_rel"], L2_rho_final=m["L2_rho_final"]))
        print(f"acoustic eps={eps}: bad={bad} W_rel={m['W_rel']*100:.3f}% "
              f"u_mid_rel={m['u_mid_rel']*100:.3f}% L2r={m['L2_rho_final']:.5f}")

    # ---- verdict ----
    ok = all(c["first_bad_step"] is None for c in sod_cases)
    for c in sod_cases:
        ok &= (c["L2_rho_final"] <= 0.03 and abs(c["W_rel"]) <= 0.03
               and abs(c["rho_mid_rel"]) <= 0.03 and abs(c["u_mid_rel"]) <= 0.03)
    # monotone convergence: L2 not increasing with resolution
    ok &= sod_cases[1]["L2_rho_final"] <= sod_cases[0]["L2_rho_final"] * 1.02
    RESULT.update(dict(verified=bool(ok), tau=0.8, nu=0.1, ny=4,
                       sod_cases=sod_cases, acoustic_supplementary=acou,
