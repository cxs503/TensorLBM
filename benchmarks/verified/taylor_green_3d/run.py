...
def auto_steps(n: int, re: float, u0: float, record_every: int,
               target_efolds: float = 10.0, cap: int = 10000,
               min_steps: int = 2000) -> int:
...
def run_case(
    ...
    for step in range(1, steps + 1):
        ...
        if step % record_every == 0:
            ...
            exs.append(ex); eys.append(ey); ezs.append(ez)
            umaxs.append(um); wmaxs.append(wm)
    ...
    return {
        ...
        "gamma_task_formula_2d": gamma_task_formula, "err_vs_task_formula_pct": err_task_pct,
        "r2": r2, "gamma_e_half1": g_h1, "gamma_e_half2": g_h2,
        "e0_theory": e0_theory, "e0_meas": energies[0],
        "ez0_meas": ezs[0], "ez_max": max(ezs), "w_max": max(wmaxs),
        "mass_drift_rel": (mass_end - mass0) / mass0,
        "n_samples": len(times) - 1,
        "wall_sec": wall,
        "times": times, "energies": energies, "exs": exs, "eys": eys,
        "ezs": ezs, "umaxs": umaxs, "wmaxs": wmaxs,
    }
...
def save_case(res: dict, out_dir: str) -> str:
    ...
    hist = np.column_stack([res["times"], res["energies"], res["exs"], res["eys"],
                            res["ezs"], res["umaxs"], res["wmaxs"]])
    np.savetxt(
        os.path.join(out_dir, f"energy_history_N{res['n']}.csv"), hist,
        header="step,energy,ex,ey,ez,umax,wmax", delimiter=",", comments="",
    )
    return name
...
def judge(cases: list[dict], tol_pct: float = 3.0) -> dict:
    ...
    return {
        "grids": ns, "err_e_pct_per_grid": dict(zip(ns, errs)),
        "err_vel_pct_per_grid": dict(zip(ns, errs_v)),
        "r2_per_grid": dict(zip(ns, r2s)),
        "converged_monotone": converged,
        "within_tol": within, "within_tol_vel": within_v,
        "exponential_r2_ok": expo, "tol_pct": tol_pct,
        "verified": verified,
        "judgment": (
            "PASS: 网格 γ_E 偏差≤3% 且随 N 收敛 → 保存 verified/"
            if verified else
            "FAIL: 未达 ≤3% 或未收敛（见各案例 err_e_pct 与收敛趋势）→ 不保存 verified/"
        ),
    }
...
def main() -> None:
    ...
    if args.scan:
        ...
        for re in (16, 24, 32, 48):
            for n in (64, 96):
                ...
                res = run_case(n, re, args.u0, steps, record_every=args.record_every,
                               dtype=args.dtype, device=args.device, threads=args.threads)
                cases.append({k: v for k, v in res.items()
                              if k not in ("times", "energies", "exs", "eys", "ezs",
                                           "umaxs", "wmaxs")})
                print(f"Re={re:3d} N={n:3d}: err_E={res['err_e_pct']:+.4f}%  "
    ...
    if args.n == 0:
        ...
        out = {
            ...
            "description": ("3D Taylor-Green vortex decay: u=(U0·sin(kx)cos(ky)cos(kz), "
                            "-U0·cos(kx)sin(ky)cos(kz), 0), k=2π/N, periodic N³ domain, "
                            "laminar viscous decay, OpenLB tgv3d-style initial field"),
            "lattice": "D3Q19", "collision": "bgk", "boundary": "periodic (stream3d 模运算, 库内建)",
            "extrap": "none",
            "common_modules": ["solver3d.collide_bgk3d", "solver3d.stream3d",
                               "d3q19.equilibrium3d", "d3q19.macroscopic3d"],
            "formula": ("gamma_vel_theory = 3·nu·k², gamma_E_theory = 6·nu·k² "
                        "(modes (±k,±k,±k), |κ|²=3k²; nu=(tau-1/2)/3, k=2π/N); "
                        "task-formula 2νk² is the 2D velocity rate — documented, not used"),
            "task_formula_note": ("任务书/问题清单 'e^{-2νk²t}' 是 2D TG（|κ|²=2k²）的速度衰减率；"
                                  "3D 场波矢 (±k,±k,±k) → |κ|²=3k²，正确对比为 γ_vel=3νk²、γ_E=6νk²。"
                                  "err_vs_task_formula_pct ≈ +200%（仅记录不作判据）。"),
            "physics_note": ("3D TG 非 NS 精确解（2D 是）：u·∇u 的旋部分立即驱动涡拉伸，"
        ...
        print("\n=== 判定 ===")
        print(f"  err_E: " + ", ".join(f"N{n}={e:+.4f}%" for n, e in
                                       zip(verdict["grids"], verdict["err_e_pct_per_grid"].values())))
        print(f"  converged_monotone={verdict['converged_monotone']}  within_tol={verdict['within_tol']}  "
    ...
    print(f"  R2={res['r2']:.6f}  half=({res['gamma_e_half1']:.4e}/{res['gamma_e_half2']:.4e})  "
