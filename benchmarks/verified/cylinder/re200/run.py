...
def run_cylinder(D, u_in, re=200, n_steps=60000, warmup_frac=0.5,
                 nx_D=40, ny_D=40, device="cuda:1", seed_amp=0.10,
    ...
    nx, ny = nx_D * D, ny_D * D
    cx, cy = 12 * D, ny / 2.0
    print(f"[Re=200] D={D} nu={nu:.5f} tau={tau:.5f} grid={nx}x{ny} "
          f"device={device} steps={n_steps} St_seed={st_seed}", flush=True)
    ...
    return {"D": D, "re": re, "nu": nu, "nx_D": nx_D, "ny_D": ny_D, "tau": tau,
            "sponge": {"x0": x0, "width": n_sponge, "alpha": alpha,
                       "power": power, "sigma": "quadratic",
                       "tau_max": float(tau_field.max())},
            "seed": {"st_seed": St_seed, "amp": seed_amp, "cols": seed_cols,
                     "n_periods": 2, "T_seed_steps": T_seed, "n_seed_steps": n_seed},
            "cd": cd_avg, "cd_half1": cd_h1, "cd_half2": cd_h2,
            "cl": cl_avg, "cl_std": cl_std,
            "st": float(st), "st_raw": float(st_raw),
            "steps_done": steps_done, "nan_at": nan_at,
            "warmup_frac": warmup_frac,
            "err_pct": (cd_avg - REF_CD) / REF_CD * 100,
            "st_err_pct": (st - REF_ST) / REF_ST * 100,
            "ref_cd": REF_CD, "ref_cd_range": [REF_CD_LO, REF_CD_HI],
            "ref_st": REF_ST, "ref_st_range": [REF_ST_LO, REF_ST_HI],
            "wall_s": time.time() - t0}
...
if __name__ == "__main__":
    ...
    r = run_cylinder(a.D, a.u, a.re, a.steps, device=a.device,
                     ...
                     save_series=a.series, st_seed=a.st_seed)
