...
def run_3d_case(cfg: dict) -> dict:
    ...
    config = DamBreak3DConfig(
        **{k: v for k, v in cfg.items() if k in DamBreak3DConfig.__dataclass_fields__}
    )
    t0 = time.time()
    try:
        ...
        return {
            ...
            "mass_stable": abs(mass_values[-1] - mass_values[-2]) < 0.01
...
def main():
    ...
    for r in results:
        ...
        print(
            f"{r['label']:<28} {r.get('tau', 0):.2f} {x01:>8} {x02:>8} {x03:>8} {mass:>7} {nan:>5} {sps:>8}"
        )
