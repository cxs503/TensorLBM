...
"""B3D-HR: 3D 后向台阶高 Re (Re=200/400) 展向周期再附着长度验证 (D3Q19)。

背景 (benchmarks/pending/bstep_3d/result.json, 2026-08):
    Re=100 展向周期 X_r/h=3.40 (+13.2% vs Armaly 3.0), 与 2D 同 Re 系统偏长
    (+9~15%) 一致 —— 低 Re 分离泡偏长是 LBM 数值误差, 高 Re 误差预期减小。
    本脚本重测 Re=200/400, 目标对比 Re=200 参考 X_r/h≈5.5 (2D 数值解,
    Armaly 实验 5.5~6), 容差 ±3%。

几何 (与 2D ny101/ny161 两档完全同口径, 3D 化):
    h=49 -> (nx,ny,nz)=(490,101,98),  x_step=98, ER=(101-2)/(101-1-49)=1.9412
    h=78 -> (780,161,156),            x_step=156, ER=159/82=1.9390
    nx=10h (上游 2h, 下游 8h), nz=2h (展向周期宽度, Lz=1~2h 任务区间内)。
    台阶块: (x<x_step)&(y<h) 全 z 层; y=0 (x>=x_step) 与 y=ny-1 为壁面; z 向周期。

入口: 充分发展抛物线 u(y)=4·U_max·yp·(H_up-yp)/H_up² (壁面 y=step_h-0.5/ny-0.5),
      H_up=ny-1-step_h, 经 3D Zou/He 速度 BC 逐点施加 (与 run.py Re=100 同款)。
出口: Zou/He 压力出口 rho=1。壁面/台阶: bounce_back_cells_3d。
流场: stream3d_roll (z 周期天然)。碰撞: bgk (默认) / mrt / rlbm (--collision)。

Re 定义 = U_max·step_h/ν (Armaly)。nu = U_max·h/Re, τ = 3ν + 0.5。
    Re=200: nu=0.01225 (h=49), τ=0.5368  -> 碰撞默认 BGK
    Re=400: nu=0.006125 (h=49), τ=0.5184 -> MRT 在 τ≈0.53 失稳 (见
            benchmarks/pending/backward_step/result.json: MRT τ=0.529 失稳),
            必须 RLBM (Latt & Chopard 2006 正则化 BGK, 方腔 Re=1000 同款修复)。

再附着点: 台阶后壁面剪切 τ_w∝ux(y=1) 过零点 (线性亚格插值), 距台阶下游立面
          x=x_step-0.5 归一化: X_r/h。主指标 = 展向平均场; 另报告各 z 平面
          xr(z) 的 std (展向均匀性)。

判定 (Re=200): 真实模拟 (无外推), |X_r/h - 5.5|/5.5 ≤ 3% 且两档网格
    (h=49/78) 收敛 (细网格 X_r/h 与粗网格差 < 0.15 且细网格误差不劣于粗网格),
    且各档稳态收敛。Re=400 为参考 (2D 经验: RLBM 稳定但分离泡巨大 ~9h+,
    8h 下游域可能不足, 如实记录)。

用法:
    python run_highre.py --re 200 --h 49 --steps 300000 --device cuda:1 [--collision bgk|mrt|rlbm] [--quick N]
    python run_highre.py --re 200 --scan --device0 cuda:1 --device1 cuda:1
"""
...
def _patch_lattice_constants(device: torch.device) -> None:
    """d3q19.C 转 fp32 常驻 CUDA (int64 广播乘法带宽减半); OPPOSITE 常驻 CUDA
    使库 bounce_back_cells_3d 的 .to(device) 变为 no-op。仅 benchmark 进程内生效。"""
...
CS2 = 1.0 / 3.0
# Re 相关参考 X_r/h (2D 数值解): Armaly 实验 Re=200 为 5.5~6, 2D 数值 ~5.5;
# Re=400 2D 数值 ~8 (参考性质)。
RE_XR_REF = {200.0: 5.5, 400.0: 8.0}
ER_REF = 1.94

# 两档几何 (与 2D ny101/ny161 完全同口径, 3D 化): h -> (nx, ny, nz, x_step)
GRIDS = {
    49: (490, 101, 98, 98),
    78: (780, 161, 156, 156),
}
...
def make_parabolic_profile(ny: int, step_h: int, u_max: float) -> np.ndarray:
    """充分发展抛物线剖面 (上游通道壁面在 y=step_h-0.5 与 y=ny-0.5, 高 H=ny-1-step_h)。
...
def outlet_pressure_bc(f: torch.Tensor, rho_out: float = 1.0) -> torch.Tensor:
    """Zou/He 压力出口 BC at x=nx-1 (rho=rho_out)。"""
    sum_cx0 = (
        f[0, :, :, -1] + f[3, :, :, -1] + f[4, :, :, -1]
        + f[5, :, :, -1] + f[6, :, :, -1]
...
def run_case(h: int, steps: int, out_interval: int, device: torch.device,
             u_max: float, re: float, collision: str, out_dir: Path,
             quick: int = 0, do_compile: bool = True, init: str = "full") -> dict:
    nx, ny, nz, x_step = GRIDS[h]
    er = (ny - 2) / (ny - 1 - h)
    nu = u_max * h / re
    tau = 3.0 * nu + 0.5
    ...
    # 初始化: rho=1 + 抛物线剖面, uy=uz=0
    #   init="full"     (默认, 与 Re=100 run.py 一致): 剖面预置全 x 域
    #   init="upstream" (低 τ 更温和): 剖面仅预置上游通道 x < x_step, 下游静止
    ...
    if init == "upstream":
        ux0 = ux0.masked_fill(
            torch.arange(nx, device=device).view(1, 1, nx) >= x_step, 0.0
        )
    ...
    collide = {
        ...
        "mrt": collide_mrt3d,
        "rlbm": collide_rlbm3d,
        "trt": lambda f, tau: collide_trt3d(f, tau, lambda_trt=3.0 / 16.0),
    }[collision]
    ...
    np.savez(
        out_dir / f"final_ux_re{int(re)}_H{h}.npz",
        ux=ux_f.detach().cpu().numpy(),
        solid=solid.detach().cpu().numpy(),
    )

    xr_ref = RE_XR_REF.get(re, 5.5)
    result = {
        "case": "bstep_3d_highre",
        "name": f"backward_facing_step_3d_re{int(re)}_H{h}",
        "reference": {
            "source": "Armaly, Durst, Pereira & Schoenung, JFM 127:473-496 (1983); "
                      "2D 数值解 (Erturk 2008 等) Re=200 X_r/h≈5.5",
        ...
        "result": {
            ...
            "xr_h_err_pct": (meas_final["xr_h"] - xr_ref) / xr_ref * 100.0,
            ...
            "ms_per_step": round(1000.0 * wall_t / max(diagnostics[-1]["step"], 1), 2) if diagnostics else None,
            "device": str(device),
...
# ---------------------------------------------------------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="3D backward-facing step high-Re benchmark")
    ap.add_argument("--h", type=int, default=49, choices=sorted(GRIDS),
                    help="step height in cells (49 -> ny=101, 78 -> ny=161)")
    ap.add_argument("--steps", type=int, default=int(os.environ.get("B3DHR_STEPS", 300000)))
    ...
    ap.add_argument("--collision", default="bgk", choices=["bgk", "mrt", "rlbm", "trt"])
    ...
    ap.add_argument("--init", default="full", choices=["full", "upstream"],
                    help="initial velocity field: full-x parabolic (default) or upstream-channel only")
    ap.add_argument("--out", default="/tmp/bstep3d_runs_highre")
    ap.add_argument("--verified", default="/home/wxsc/cxs/TensorLBM/benchmarks/verified/bstep_3d")
    ap.add_argument("--scan", action="store_true", help="run h=49 and h=78, write convergence summary")
    ap.add_argument("--device0", default="cuda:1")
    ap.add_argument("--device1", default="cuda:1")
    args = ap.parse_args()

    err_tol_pct = 3.0
    xr_ref = RE_XR_REF.get(args.re, 5.5)

    if args.scan:
        jobs = [(49, torch.device(args.device0)), (78, torch.device(args.device1))]
    else:
    ...
    for h, device in jobs:
        ...
        print(f"=== B3D-HR BFS Re={args.re} H={h} steps={steps} device={device} "
              f"collision={args.collision} compile={'on' if not args.no_compile else 'off'} ===",
              flush=True)
        r = run_case(h, steps, args.interval, device, args.u_max, args.re,
                     args.collision, Path(args.out), quick=args.quick,
                     do_compile=not args.no_compile, init=args.init)
        cases.append(r)
    ...
    for c in cases:
        h = c["geometry"]["step_h"]
        re_i = int(c["physics"]["re"])
        with (vdir / f"result_re{re_i}_H{h}.json").open("w", encoding="utf-8") as fh:
    ...
    if args.scan and len(cases) >= 2:
        ...
        all_finite = all(c["result"]["finite"] for c in cases)
        passed = (all_finite and all(abs(e) <= err_tol_pct for e in errs)
                  and grid_converged and per_case_ok)
        summary = {
            "case": f"bstep_3d_re{int(args.re)}_convergence",
            "name": f"backward_facing_step_3d_re{int(args.re)}_spanwise_periodic",
            "reference_xr_h": xr_ref,
            "err_tol_pct": err_tol_pct,
            "collision": args.collision,
            "grids": [c["geometry"] for c in cases],
            "tau": [c["physics"]["tau"] for c in cases],
            "xr_h": xrs,
            "err_pct": errs,
            ...
            "notes": (
                ...
                "细网格误差不劣于粗网格, 且各档稳态收敛且有限。"
                "背景: Re=100 展向周期 +13.2% (低 Re 分离泡偏长); 高 Re 误差预期减小。"
            ),
        }
        with (vdir / f"result_re{int(args.re)}.json").open("w", encoding="utf-8") as fh:
        ...
        print(f"-> {vdir / f'result_re{int(args.re)}.json'}", flush=True)
    else:
        h = cases[0]["geometry"]["step_h"]
        re_i = int(cases[0]["physics"]["re"])
        print(f"-> {vdir / f'result_re{re_i}_H{h}.json'}", flush=True)
