...
def run_case(grid_name: str, U: float, nu: float, n_steps: int,
             out_path: str, device_str: str = "cpu") -> dict:
    g = GRIDS[grid_name]
    nx, ny = g["nx"], g["ny"]
    ...
    l2_rel = float(np.linalg.norm(rel_err[m]) / np.linalg.norm(np.ones(m.sum()))) if m.any() else float("nan")
    ...
    sym_err_pct = float(np.max(np.abs(u_lo[:py - 1][::-1] - u_hi[:py - 1]) / np.maximum(U, 1e-12)) * 100.0)
    ...
    result = {
        ...
        "extrap": "none",
        "nx": nx, "ny": ny, "plate_y": py, "plate_len": plate_len,
        "U": U, "nu": nu, "tau": tau,
        "x0_leading_edge": le,
        "x_probe": probe, "x_eff": x_eff, "Rex": Rex,
...
def main() -> None:
    ...
    keys = ["grid", "nx", "ny", "plate_y", "U", "nu", "tau", "x_eff", "Rex", "n_steps",
            "umax_drift_last_2000", "l2_rel_err_profile", "max_rel_err_profile_pct",
            "eta_tab_comparison", "u_edge_over_U_probe", "symmetry_max_dev_over_U_pct",
            "delta_star_meas", "delta_star_ref_1p7208_x_over_sqrtRex",
            "Cf_sim", "Cf_ref_0p664_over_sqrtRex", "Cf_err_pct",
