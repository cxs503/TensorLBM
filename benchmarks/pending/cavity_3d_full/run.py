...
"""全 3D 方腔（5 壁固定 + 顶盖移动，Lz=H 立方腔）Re=1000 — 3D 涡结构验证。
...
碰撞：collide_rlbm3d（Re=1000 下 MRT 在等 Re 配方 tau=3*u_lid*nx/Re+0.5 下
      不稳定——verified cavity/re1000 实测 MRT 稳定上限约 Re~576
      (u_lid=0.06, tau>=0.56)；RLBM 正则化滤除高阶鬼矩，tau->0.5 稳定）。
...
已知口径差异（记录于 /tmp/cavity3d_full_gap.md）：
  - 顶盖角点：zou_he_moving_lid_3d 覆盖整层含角点强制 ux=u_lid；Ku 1987
    顶盖 x 角点设 u=0（角点速度奇异处理不同）→ 角点局部差异属预期
  - 中心平面 3D 效应 vs 2D 不是误差，是物理（侧壁边界层）→ 3D 修正量单独报告
...
from tensorlbm.solver3d import collide_rlbm3d, stream3d
...
def main():
    ...
    ap.add_argument("--threads64", type=int, default=48)
    ap.add_argument("--threads96", type=int, default=32)
