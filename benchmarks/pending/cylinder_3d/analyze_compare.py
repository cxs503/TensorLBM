...
def main() -> int:
    ...
    print(f"{'formula':14s} | " + " | ".join(
        f"D{r['D_cells']}: cd_f   cd    err%" for r in grids))
    best = {}
    for k in FORMULAS:
        cells = []
        for r in grids:
        ...
        print(f"{k:14s} | " + " | ".join(
            f"{cf:.3f} {ct:.3f} {err:+.2f}" for cf, ct, err in cells))

    print("\n--- per-grid details ---")
    for r in grids:
        D = r["D_cells"]
        print(f"D={D}: near={r['n_near_cells']} faces={r['n_wall_faces']} "
        ...
        if "per_cell_diag" in r:
            d = r["per_cell_diag"]
            print(f"    diag: yface={d['n_y_face_cells']} xonly={d['n_x_only_cells']} "
    ...
    if len(big) >= 2:
        ...
        print(f"gate: |err|<=3% at both grids AND "
              f"|Cd60-Cd40|/ref<=3% (convergence)")
        for k in FORMULAS:
            ...
            print(f"  {k:14s} err40={e40:+.2f}% err60={e60:+.2f}% "
