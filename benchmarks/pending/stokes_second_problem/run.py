...
_BENCH_DIR = next((p for p in Path(__file__).resolve().parents
                   if (p / "compile_route.py").exists()), None)
...
def run_case(
    ...
    for rec in records:
        ...
        rel = np.divide(np.abs(u_num - u_ana), np.abs(u_ana),
                        out=np.zeros_like(u_ana), where=mask)
        rel5 = np.divide(np.abs(u_num - u_ana), np.abs(u_ana),
                         out=np.zeros_like(u_ana), where=mask5)
        per_step.append({
            "t_lb": t,
            "cycle_k": rec["k"],
...
def period_convergence(r: dict, U0: float, tol_pct: float = 0.5) -> dict:
    ...
    for phi, ss in by_phase.items():
        ...
        u_e = np.array(early["u_profile"]); u_l = np.array(late["u_profile"])
        u_a = np.array(early["u_analytic"])
        mask = np.abs(u_a) > 0.05 * U0
        diff = np.max(np.abs(u_e - u_l)[mask]) / U0 * 100.0
        worst = max(worst, diff)
        rows.append({"phase": phi,
                     "t_early": early["t_lb"], "t_late": late["t_lb"],
...
def compare_grids(r_lo: dict, r_hi: dict, U0: float, tol_pct: float = 3.0) -> dict:
    ...
    k_lo = r_lo["cycles"][1]; k_hi = r_hi["cycles"][1]
    rows = []
    err_lo, err_hi = [], []
    for phi in PHASES:
        slo = m_lo[(round(phi, 6), k_lo)]
        shi = m_hi[(round(phi, 6), k_hi)]
        err_lo.append(slo["max_rel_err_pct"]); err_hi.append(shi["max_rel_err_pct"])
        y_lo = np.array(slo["y_profile"]); u_lo = np.array(slo["u_profile"])
        y_hi = np.array(shi["y_profile"]); u_hi = np.array(shi["u_profile"])
    ...
    return {"rows": rows, "converged": converged,
            "max_rel_err_late_H40": err_lo, "max_rel_err_late_H80": err_hi}
...
def main() -> None:
    ...
    p1.add_argument("--cycles", type=int, nargs=2, default=None,
    ...
    p2.add_argument("--cycles", type=int, nargs=2, default=None,
    ...
    if args.mode == "single":
        ...
        r = run_case(args.H, args.tau, args.U, cycles, args.nx,
                     compile_mode=compile_mode)
        Path(args.out_json).write_text(json.dumps(r, indent=2))
        print(f"H={r['H']} delta={r['delta_lb']} omega={r['omega_lb']:.6f} "
              f"T={r['T_period_lb']:.1f} steps={r['max_step']}")
        for s in r["per_step"]:
            print(f"  k={s['cycle_k']} phi={s['phase_nominal']:.4f} t={s['t_lb']:6d}  "
        ...
        print(f"  steady-state: worst cycle-profile diff = {pc['worst_pct']:.4f}% U0  "
              f"steady={pc['steady']}")
        print(f"  mass_drift={r['mass_drift_pct']:.2e}% finite={r['finite']} "
        ...
        summary = {
            ...
            "per_grid": [{k: r[k] for k in
                          ["H", "ny", "nx", "tau", "nu_lb", "delta_lb", "omega_lb",
                           "T_period_lb", "U0", "Ma", "cycles", "max_step",
            ...
            "status": "VERIFIED" if (passed and conv["converged"] and all(pc["steady"] for pc in pcs))
                      else "NOT_PASSED",
        }
        ...
        for row in conv["rows"]:
            print(f"  phi={row['phase']:.4f}  profile_diff_H40vsH80 = "
