...
def build_shells(n: int, device: torch.device):
    ...
    kmax = int(idx.max())
    return (torch.from_numpy(idx).to(device), torch.from_numpy(w).to(device), kmax)
...
def velocity_derivative_skewness(
    ux: torch.Tensor, uy: torch.Tensor, uz: torch.Tensor, n: int
) -> float:
    ...
    dux_dx = torch.fft.irfftn(
        torch.fft.rfftn(ux, dim=(0, 1, 2), norm="ortho") * (1j * kx).view(1, 1, -1),
        s=(n, n, n),
        dim=(0, 1, 2),
        norm="ortho",
    )
...
def run_case(
    ...
    out = {
        ...
        "umax": umax_list,
        "kmax": kmax,
        "spectra": spec_arr.astype(np.float32),
    }
    return out
...
def fit_decay_exponent(
    t: np.ndarray, E: np.ndarray, t_lo_frac: float = 0.15, t_hi_frac: float = 0.6
) -> dict:
...
def fit_spectrum_slope(
    specs: np.ndarray, times: np.ndarray, kmin: int, kmax: int, t_lo: float, t_hi: float
) -> dict:
    ...
    r2 = float(
        ...
        - resid
        @ resid
        / ((np.log(Sbar[m]) - np.log(Sbar[m]).mean()) @ (np.log(Sbar[m]) - np.log(Sbar[m]).mean()))
    )
...
def judge(results: list[dict], tol_pct: float = 3.0) -> dict:
    ...
    slope_errs = {n: (slopes[n] - KOLMOGOROV_SLOPE) / abs(KOLMOGOROV_SLOPE) * 100.0 for n in ns}
    ...
    conv_slope = (
        len(ns) >= 2
        and abs(slopes[ns[1]] - slopes[ns[0]]) / abs(KOLMOGOROV_SLOPE) * 100.0 <= tol_pct
    )
    conv_n = (
        len(ns) >= 2
        and n_dec[ns[0]] != 0
        and abs(n_dec[ns[1]] - n_dec[ns[0]]) / abs(n_dec[ns[0]]) * 100.0 <= tol_pct
    )
    ...
    return {
        ...
        "converged_slope": conv_slope,
        "converged_n": conv_n,
        "tol_pct": tol_pct,
        "verified": verified,
        "judgment": (
            "PASS: 两档 -5/3 斜率≤3% 且 n∈[1.2,1.5] 且档间收敛 → 保存 verified/"
            if verified
            else "FAIL: 未达 ≤3%/物理带/档间收敛 → 不保存 verified/（记录未达标）"
        ),
    }
...
def main() -> None:
    ...
    ap.add_argument(
        "--cw", type=float, default=0.325, help="WALE constant (Nicoud & Ducros standard Cw=0.325)"
    )
    ap.add_argument(
        "--cv",
        type=float,
    ...
    ap.add_argument(
        "--collision", choices=["bgk", "mrt", "wale", "wale_mrt", "vreman"], default="bgk"
    )
    ...
    for n in grids:
        ...
        print(
            f"=== N={n}³  u_rms={args.u_rms} m0={args.m0} tau={args.tau} "
        ...
        for i, tt in enumerate(t[1:], start=1):
            ...
            if mm.sum() >= 3:
                slopes_ev.append(
                    [float(tt), float(np.polyfit(np.log(kk[mm]), np.log(row[mm]), 1)[0])]
                )
        ...
        np.savez(
            os.path.join(args.out, f"data_N{n}.npz"),
            times=t,
            energies=E,
            spectra=spec_arr,
            urms=np.asarray(res["urms"]),
            umax=np.asarray(res["umax"]),
        )
        ...
        print(
            f"  L_int={res['L_int']:.2f} Re_L={res['re_L_molecular']:.0f} "
            f"τ_L={tau_L:.0f} Re_λ={res['re_lambda_molecular']:.0f}",
            flush=True,
        )
        print(
            f"  E0={res['E0']:.3e} E_end={res['E_end']:.3e} "
            f"decay={res['E_decay_factor']:.1f}x  wall={res['wall_sec']:.0f}s "
            f"({res['ms_per_step']:.1f} ms/step)",
            flush=True,
        )
        print(
            f"  decay n={df['n']:.4f} (R²={df['r2']:.4f}, half {df['n_half1']:.3f}/"
            f"{df['n_half2']:.3f}, vo={df['n_virtual_origin']:.3f})",
            flush=True,
        )
        print(
            f"  slope[{kmin},{kmax}] t∈[{t_dev:.0f},{t_end:.0f}] = {sf['slope']:.4f} ± "
        ...
        print(
            f"  skewness S3 mean={np.mean(s3s):.3f} (n={len(s3s)}, "
    ...
    out = {
        ...
        "description": (
            "3D decaying isotropic turbulence (DIT): random solenoidal IC with "
        ...
        "common_modules": [
            "solver3d.stream3d",
            "turbulence.collide_smagorinsky_bgk3d",
            "d3q19.equilibrium3d",
            "d3q19.macroscopic3d",
        ],
        "analysis_tools": [
            "energy_spectrum (3D FFT shell sum, run.py 自实现, 库无 — G-DIT-1)",
    ...
    print(
        "  slope err: "
        + ", ".join(f"N{n}={e:+.2f}%" for n, e in verdict["slope_err_pct_per_grid"].items())
    )
    print("  n_decay: " + ", ".join(f"N{n}={v}" for n, v in verdict["n_decay_per_grid"].items()))
    print(
        f"  within_slope={verdict['within_slope_tol']}  n_in_band={verdict['n_in_physical_band']}  "
