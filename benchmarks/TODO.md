...
- **B50 多孔介质渗透率**（Wave-4，2026-09-21）：fs=10 协议 φ=0.3 三档 **6.53→2.86→1.38%**、φ=0.5 五档 **73.87→27.19→10.39→3.54→0.0484%** 全严格单调收敛（Sangani–Acrivos 1982；k_ref=d²/((1−φ)f) 归一化三路定裁；fs=1 链 fp32 噪声地板伪差披露）→ benchmarks/verified/permeability/
...
- **B48 两相 Poiseuille / B49 Washburn（Wave-4 多孔介质弧，2026-09-21）**：SC-MCMP 模型点结构性 FAIL（26.8→33.2% 反收敛；β 19.08%/3.17% + 绝对判据 +689%/+949% + W128@164 NaN；σ 不可测）——判决记录 pending/two_phase_poiseuille/ 与 pending/capillary_invasion_washburn/；库机械缺陷 6 项已修 PR #305，深物理 3 项（分量自速度平衡/吸附泵/W≳64 发散）xfail 锁证
...
> owner 2026-09-20 "需要通过共性模块实现，按照小于3%误差标准，并行推进开发"——
> 库有 porous_media/porous_media3d 模块但基准为零，Wave-4 = 多孔介质弧。
> W4-A 渗透率（verified/permeability）、W4-B 两相 Poiseuille + Washburn（双
> pending）、W4-C φ=0.5 阶梯外延（fp32 噪声地板发现 → 修正案 #4 协议
> fs 1→10）、W4-D 库机械修复（PR #305）。工件暂存
> /nfs/wangxi/runs/bm_widen_w4_20260920/。库缺陷 10 条清单坐实，6 条机械项
> PR #305 已修（G_12 符号/Washburn 2β/dp_inlet/mu_ratio/0.05 台阶×4/周期缝），
> 深物理 3 项 xfail-strict 锁证据（分量自速度平衡/吸附组分泵/W≳64 发散）。

| B48 | 两相 Poiseuille（SC-MCMP 双组分，τ=(1.0,0.75) 名义 M=2，G_12=−2.5 全交换，H64/H128） | ❌ **FAIL（反收敛）**——pending/two_phase_poiseuille/ 判决记录。中心区 max rel err **26.82%/33.17%**（阈值 9–11 倍）且细化恶化；归因=SC-MCMP 结构性动量耦合偏差（`collide_sc_two_component` 各分量用自身速度构造平衡态；非 Ma/互溶度/驱动强度，G_x 2e-5 敏感性证不变）；实测黏度比重标定被预注册禁止未做。库缺陷 #1（G_12 符号挡死唯一可用域）/#5（30% 互溶度致有效 M 反转 0.54）与本记录直接相关，机械项已随 PR #305 修，核心核另立项 |
| B49 | 毛管侵入 / Washburn 动力学（SC-MCMP，W32/W64/W128，修正案 #2 协议：固 seam+气储层+水汇+K=1 定尺寸） | ❌ **FAIL（三判据全败）**——pending/capillary_invasion_washburn/ 判决记录。(a) β=0.5954/0.5159（19.08%/3.17% 压线如实 FAIL）；(b) 前沿 L(t) vs 名义 ODE **+689%/+949%**（前沿速度=设计值 15–25×，SC churn u_max≈0.073 主导，β≈0.5 系汇钉住饱和形副产物）；W128 **第 164 步确定性 NaN**（稳定包络 W≤64）；σ 全交换构造下不可测（ρ_tot≡1）→ 毛管项无法构造；θ=83.33° 自测稳定；40k 步协议偏差（锁 30k/20k/15k）已披露 |
| B50 | 多孔介质渗透率（2D 周期圆柱方阵 Stokes，D2Q9 BGK τ=1.0，k_sim=ν⟨u_x⟩_fluid/a vs Sangani–Acrivos 1982 表） | ✅ **已入库（fs=10 主判据）** verified/permeability/。φ=0.3 三档 **6.53→2.86→1.38%**、φ=0.5 五档 **73.87→27.19→10.39→3.54→0.048%** 全严格单调最细 ≤3%；fs=1 原链并列披露（φ=0.5 d320 19.92% 非单调=fp32 噪声地板伪差，修正案 #4 先于 formal 写入哈希链）。**fp32 噪声地板发现**：a_body∝d⁻³ 触底，d320/fs1 注入比 r=1.18513 恰=观测超出 461.18/389.14，三尺度闭合 389.139/389.311/389.203（0.044%）；探针校正链 φ=0.3 6.54→2.83→1.35%、φ=0.5 …→1.21% 同样单调 ≤3%（判决与校正无关，判定用实测值）；归一化修正 k_ref=d²/((1−φ)f) 三路独立定裁（Basilisk 源码+应力推导+FD 解算器）；d160 档 3.54%（>3%，锁定规则只要求最细档）已披露 |
