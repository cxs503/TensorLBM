...
| B33 | DIT 衰减各向同性湍流（D3Q19 周期域 + Smagorinsky LES Cs=0.12，随机无散 IC E(k)∝k⁴exp(−2(k/3)²)，N=64/128，15000 步） | ❌ **未达标**（2026-08-19 正式两档验证，pending/dit_turbulence/）：两档斜率各自 ≤3%（N64 −1.646/+1.2%、N128 −1.639/+1.7%）、档间斜率收敛（0.46%），但 **N=64 阻塞**——衰减指数 n=1.610 超物理带 [1.2,1.5]、n 档间差 10%、斜率强窗口敏感（[6,14] 随时间窗变陡 −1.65→−2.61，任务指定窗 [10,24] 误差 −52%~−155%，无 −5/3 平台）；N=128 单档达标（n=1.449、S3≈−0.36）。根因：N=64 上 LES 分辨率不足（亚格子耗散相对过强，E 衰减 558× vs 142×）。建议：N=128/256 两档重验或 N=64 换更小 Cs |
