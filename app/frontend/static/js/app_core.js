...
const TAB_SEQUENCE = ['dashboard', 'cad', 'preprocess', 'solve', 'postprocess', 'benchmarks', 'compare', 'ai-flow', 'suboff', 'simulations', 'orchestration', 'agent', 'cylinder'];
const uiState = {
  activeTab: 'dashboard',
  jobsSearch: '',
  jobsStatus: 'all',
...
async function loadStatus() {
  try {
    ...
    const accelInfo = s.sdaa_available
      ? `<span class="text-success"><i class="bi bi-gpu-card"></i> ${s.sdaa_count} SDAA(s): ${s.sdaa_names.join(', ')}</span>`
      : (s.cuda_available
    ...
    el.innerHTML = `
      <table class="table table-sm mb-0">
        ...
        <tr><th>${t('dashboard.accelerator')}</th><td>${accelInfo}</td></tr>
