...
const SIM_TYPES = {
  ...
  conjugate_ht: {
    label: 'Conjugate Heat Transfer (2D)',
    desc: 'Fluid-solid coupled heat transfer with Boussinesq buoyancy and solid conduction.',
    endpoint: '/api/solve/conjugate-ht',
    fields: [
      {name:'nx', label:'nx', type:'number', default:120, min:20},
      {name:'ny', label:'ny', type:'number', default:64, min:20},
      {name:'solid_x_start', label:'Solid x start', type:'number', default:48, min:0},
      {name:'solid_x_end', label:'Solid x end', type:'number', default:72, min:1},
      {name:'solid_y_start', label:'Solid y start', type:'number', default:20, min:0},
      {name:'solid_y_end', label:'Solid y end', type:'number', default:44, min:1},
      {name:'tau_f', label:'Fluid tau', type:'number', default:0.6, step:0.01, min:0.51},
      {name:'kappa_f', label:'Fluid diffusivity', type:'number', default:0.1666667, step:0.001, min:0.0001},
      {name:'alpha_s', label:'Solid diffusivity', type:'number', default:0.05, step:0.001, min:0.0001},
      {name:'k_ratio', label:'k_s / k_f', type:'number', default:5.0, step:0.1, min:0.1},
      {name:'T_hot', label:'Hot temperature', type:'number', default:1.0, step:0.1},
      {name:'T_cold', label:'Cold temperature', type:'number', default:0.0, step:0.1},
      {name:'Q_source', label:'Solid heat source', type:'number', default:0.0, step:0.001},
      {name:'beta', label:'Boussinesq beta', type:'number', default:0.002, step:0.0001, min:0},
      {name:'gravity', label:'Gravity', type:'number', default:0.00002, step:0.00001, min:0},
      {name:'n_steps', label:'Time steps', type:'number', default:500, min:10},
      {name:'output_interval', label:'Output interval', type:'number', default:100, min:1},
      {name:'device', label:'Device', type:'device'},
    ],
  },
  cumulant_cylinder_flow: {
    label: 'Cumulant Cylinder Flow (2D)',
    desc: 'High-stability cumulant LBM for higher-Re cylinder wake.',
    endpoint: '/api/solve/cumulant-cylinder-flow',
    fields: [
      {name:'nx', label:'nx', type:'number', default:200, min:20},
      {name:'ny', label:'ny', type:'number', default:100, min:20},
      {name:'re', label:'Reynolds number', type:'number', default:500, min:1},
      {name:'u_in', label:'Inlet velocity', type:'number', default:0.05, step:0.01, min:0.001},
      {name:'radius', label:'Cylinder radius', type:'number', default:10, min:1},
      {name:'omega_b', label:'omega_b', type:'number', default:1.0, step:0.01, min:0.01, max:2},
      {name:'omega_3', label:'omega_3', type:'number', default:1.0, step:0.01, min:0.01, max:2},
      {name:'omega_4', label:'omega_4', type:'number', default:1.0, step:0.01, min:0.01, max:2},
      {name:'n_steps', label:'Time steps', type:'number', default:2000, min:100},
      {name:'output_interval', label:'Output interval', type:'number', default:500, min:10},
      {name:'device', label:'Device', type:'device'},
    ],
  },
  sliding_mesh_rotor: {
    label: 'Sliding Mesh Rotor (2D)',
    desc: 'Rotor-stator sliding-interface simulation for rotating machinery.',
    endpoint: '/api/solve/sliding-mesh-rotor',
    fields: [
      {name:'nx', label:'nx', type:'number', default:256, min:64},
      {name:'ny', label:'ny', type:'number', default:256, min:64},
      {name:'u_tip', label:'Tip speed', type:'number', default:0.05, step:0.01, min:0.001},
      {name:'re', label:'Reynolds number', type:'number', default:200, min:1},
      {name:'rotor_radius', label:'Rotor radius frac', type:'number', default:0.35, step:0.01, min:0.05, max:0.48},
      {name:'blade_radius', label:'Blade radius frac', type:'number', default:0.2, step:0.01, min:0.02, max:0.35},
      {name:'n_blades', label:'Blade count', type:'number', default:4, min:2, max:8},
      {name:'n_steps', label:'Time steps', type:'number', default:2000, min:1},
      {name:'output_interval', label:'Output interval', type:'number', default:400, min:1},
      {name:'priority', label:'Priority', type:'number', default:5, min:1, max:10},
      {name:'device', label:'Device', type:'device'},
    ],
  },
  passive_scalar_transport: {
    label: 'Passive Scalar Transport (2D)',
    desc: 'D2Q9 flow + scalar transport for species or tracer dispersion.',
    endpoint: '/api/solve/passive-scalar-transport',
    fields: [
      {name:'nx', label:'nx', type:'number', default:256, min:32},
      {name:'ny', label:'ny', type:'number', default:128, min:32},
      {name:'u_in', label:'Inlet velocity', type:'number', default:0.05, step:0.01, min:0.001},
      {name:'re', label:'Reynolds number', type:'number', default:100, min:1},
      {name:'diffusivity', label:'Scalar diffusivity', type:'number', default:0.01, step:0.001, min:0.0001},
      {name:'src_cx', label:'Source x (frac)', type:'number', default:0.15, step:0.01, min:0, max:1},
      {name:'src_cy', label:'Source y (frac)', type:'number', default:0.5, step:0.01, min:0, max:1},
      {name:'src_radius', label:'Source radius (frac)', type:'number', default:0.04, step:0.01, min:0.001, max:0.25},
      {name:'cyl_cx', label:'Cylinder x (frac)', type:'number', default:0.25, step:0.01, min:0, max:1},
      {name:'cyl_cy', label:'Cylinder y (frac)', type:'number', default:0.5, step:0.01, min:0, max:1},
      {name:'cyl_radius', label:'Cylinder radius (frac)', type:'number', default:0.06, step:0.01, min:0.001, max:0.25},
      {name:'n_steps', label:'Time steps', type:'number', default:3000, min:1},
      {name:'output_interval', label:'Output interval', type:'number', default:500, min:1},
      {name:'priority', label:'Priority', type:'number', default:5, min:1, max:10},
      {name:'device', label:'Device', type:'device'},
    ],
  ...
  cavitation_flow: {
    label: 'Cavitation Flow (2D)',
    desc: 'Shan-Chen + Schnerr-Sauer cavitation in nozzle-like flow.',
    endpoint: '/api/solve/cavitation-flow',
    fields: [
      {name:'nx', label:'nx', type:'number', default:256, min:64},
      {name:'ny', label:'ny', type:'number', default:128, min:32},
      {name:'G', label:'Coupling G', type:'number', default:-5.5, step:0.1, min:-8.0, max:-3.0},
      {name:'rho_init', label:'Initial density', type:'number', default:1.5, step:0.1, min:0.1},
      {name:'throat_x', label:'Throat x (frac)', type:'number', default:0.35, step:0.01, min:0.1, max:0.9},
      {name:'throat_height', label:'Throat height (frac)', type:'number', default:0.3, step:0.01, min:0.05, max:0.9},
      {name:'u_in', label:'Inlet velocity', type:'number', default:0.04, step:0.01, min:0.001},
      {name:'re', label:'Reynolds number', type:'number', default:500, min:1},
      {name:'n_steps', label:'Time steps', type:'number', default:5000, min:1},
      {name:'output_interval', label:'Output interval', type:'number', default:500, min:1},
      {name:'priority', label:'Priority', type:'number', default:5, min:1, max:10},
  ...
  oscillating_airfoil: {
    label: 'Oscillating Airfoil (2D)',
    desc: 'Moving-boundary pitch-plunge airfoil/cylinder benchmark.',
    endpoint: '/api/solve/oscillating-airfoil',
    fields: [
      {name:'nx', label:'nx', type:'number', default:512, min:128},
      {name:'ny', label:'ny', type:'number', default:256, min:64},
      {name:'u_in', label:'Inlet velocity', type:'number', default:0.05, step:0.01, min:0.001},
      {name:'re', label:'Reynolds number', type:'number', default:500, min:1},
      {name:'geom', label:'Geometry', type:'select', default:'airfoil', options:['airfoil','cylinder']},
      {name:'chord', label:'Chord', type:'number', default:60, step:1, min:10},
      {name:'n_markers', label:'Marker count', type:'number', default:120, min:20},
      {name:'A_h_frac', label:'Plunge amplitude/chord', type:'number', default:0.25, step:0.01, min:0, max:1},
      {name:'A_alpha_deg', label:'Pitch amplitude (deg)', type:'number', default:15, step:0.1, min:0, max:45},
      {name:'f_red', label:'Reduced frequency', type:'number', default:0.0015, step:0.0001, min:0.0001, max:0.02},
      {name:'phase_shift_deg', label:'Phase shift (deg)', type:'number', default:90, step:1, min:0, max:360},
      {name:'n_steps', label:'Time steps', type:'number', default:4000, min:1},
      {name:'output_interval', label:'Output interval', type:'number', default:500, min:1},
      {name:'priority', label:'Priority', type:'number', default:5, min:1, max:10},
      {name:'device', label:'Device', type:'device'},
...
const MODEL_CAPABILITIES = {
  ...
  conjugate_ht: { flow_types:['thermal'], turbulence:['none'], multiphase:['none'], schemes:['bgk'] },
  cumulant_cylinder_flow: { flow_types:['single_phase'], turbulence:['none'], multiphase:['none'], schemes:['mrt'] },
  sliding_mesh_rotor: { flow_types:['single_phase'], turbulence:['none'], multiphase:['none'], schemes:['mrt'] },
  passive_scalar_transport: { flow_types:['single_phase'], turbulence:['none'], multiphase:['none'], schemes:['bgk'] },
  cavitation_flow: { flow_types:['multiphase'], turbulence:['none'], multiphase:['sc'], schemes:['bgk'] },
  oscillating_airfoil: { flow_types:['single_phase'], turbulence:['none'], multiphase:['none'], schemes:['mrt'] },
};
...
function initPhysicsLayer() {
  ...
  selectOptions('physics-flow-type', ['single_phase','multiphase','free_surface','thermal'], {
