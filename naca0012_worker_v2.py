"""NACA 0012 airfoil drag validation — D3Q19 MRT+Smag + wall_function + far_field.

Uses the library wall_function_3d from wall_model.py and standard BC functions.

Usage:
    PYTHONPATH=src python naca0012_worker_v2.py <device_id> <angle_deg> <Cs>

Writes result to /tmp/naca0012_results/result_<device_id>_a<angle>_cs<Cs>.json
"""
...
def naca0012_half_thickness(x_over_c: torch.Tensor, t: float = 0.12) -> torch.Tensor:
    ...
    y/c = t/0.2 * (0.2969·√(x/c) - 0.1260·(x/c) - 0.3516·(x/c)²
                    + 0.2843·(x/c)³ - 0.1015·(x/c)⁴)
    """
...
def build_naca0012_mask(
    ...
    The airfoil profile lies in the x-z plane, extruded across full y span.
    True = solid.
    """
    ...
    in_profile = (zz >= z_lower) & (zz <= z_upper)
    solid = in_chord & in_profile

    # Fill LE/TE gaps (half_t → 0)
    le_col = int(cx_le)
...
def main():
    ...
    # Reference area for 2D Cd = chord (per unit span).  Our 3D sim has ny=8 span
    # cells, so Cd = total_force / (0.5·ρ·U² · chord · ny)
    ...
    for step in range(1, n_steps + 1):
        ...
        # Use library wall_function_3d
        f, df, dp = wall_function_3d(f, solid, nu, y_val=0.5)

        # Far-field BC with angled inflow (ux, uz)
        f = far_field_bc_3d(f, u_in=ux_in, uz=uz_in)

        # Override y-faces with free-slip (symmetry) for quasi-2D
