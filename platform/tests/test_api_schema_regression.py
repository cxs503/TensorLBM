...
def test_openapi_exposes_new_jobs_control_endpoints(client):
...
def test_openapi_preprocess_random_porosity_uses_sigma(client):
    schema = client.get("/openapi.json").json()
...
def test_openapi_solver_cylinder_has_physics_field(client):
    schema = client.get("/openapi.json").json()
