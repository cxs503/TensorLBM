    ...
    def _run(job: job_manager.Job) -> dict:
        ...
        if "geometry_library" in params.cases:
            ...
            results["geometry_library"] = {
                ...
                "ship_ok": all(bool(item["pass"]) for item in ship_entries),
                "suboff_ok": (
                    solid_cells == sorted(solid_cells) and len(set(solid_cells)) == len(solid_cells)
                ),
                "all_ok": (
                    all(bool(item["pass"]) for item in ship_entries)
                    and solid_cells == sorted(solid_cells)
