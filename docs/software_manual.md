...
4. [船舶与海洋工程算例 / Marine Benchmarks](#4-船舶与海洋工程算例--marine-benchmarks)
   ...
   - [4.5 威格利船体绕流 / 3D Wigley Hull Flow](#45-威格利船体绕流--3d-wigley-hull-flow)
...
### 4.5 威格利船体绕流 / 3D Wigley Hull Flow

#### 物理背景 / Physical Background

威格利（Wigley）抛物线船体是**船舶阻力数值计算**中使用最广泛的验证几何体。本算例采用 D3Q19 格子、**Smagorinsky MRT LES 模型**，在 Re = 200 下模拟矩形槽道中的三维粘性绕流，输出阻力系数 C_D、侧向力 C_S、垂向升力 C_L 及俯仰力矩 M_y。

The Wigley parabolic hull is the most widely used benchmark geometry for ship resistance calculation. This benchmark uses D3Q19 with **Smagorinsky MRT LES** at Re = 200 in a rectangular channel, outputting drag C_D, sideforce C_S, lift C_L, and pitching moment M_y.
...
|----|----------|----------------|
| 阻力系数 C_D | **0.95** | C_D > 0 ✓ |
| 垂向升力 C_L | **0.032** | \|C_L\| < \|C_D\| ✓（上下对称性）|
| 侧向力 C_S | **≈ 0** | C_S ≈ 0 ✓（左右对称性）|

> **注**: Re = 200 时 LBM 动量交换法计算的 C_D 包含完整的粘性阻力，参考值随域尺寸和阻塞比不同而变化。物理一致性检验（拖力方向正确、力矩对称）是工程验证的必要条件。
...
cfg = ShipHullFlowConfig(
    ...
    output_root="outputs", run_name="wigley_waves", overwrite=True,
)
...
PYTHONPATH=src python examples/ship_hull_flow.py \
  ...
  --re 200 --n-steps 4000 --output-interval 200 \
...
| 5 | Wigley 船体 Re=200 | 80×40×30 | 2 000 | C_D > 0 | 0.95 | — | 一致性✓ | ✓ |
