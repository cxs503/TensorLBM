...
**参考（Schiller–Naumann Re=100）＝ `1.0917`；"误差"列以此为准。**

| 配置 | 收敛值 | 误差 | 说明 |
|---|---|---|---|
| **单卡 D3Q19 cumulant（P3, 8000 步）** | **`1.2432`** | **+13.9%** | ✅ **达标（<15%），当前最佳路径** |
| 单卡 D3Q27 LES/WALE（800 步） | `1.4667` | +34.4% | 收敛确认（step600–800 稳定 1.481→1.467） |
| 单卡 D3Q27 cumulant（1200 步） | `0.5546` | −49.2% | 收敛确认（step1100–1200 稳定 0.5551→0.5546） |
| **集成 D3Q27 cumulant（no-refreeze, 1200 步）** | **`0.5458`** | −50.0% | ✅ **与单卡 cumulant 对齐（差 1.6%）⇒ 集成架构正确** |
| 集成 D3Q27 cumulant（refreeze 旧默认, 2000 步） | `2.7255` | +149.7% | ❌ 已被 d.6 的 refreeze 修复取代 |
| 集成 D3Q27 cumulant（no-refreeze, 150 步瞬态） | `2.0015` | +83.3% | 未收敛，仅用于 A/B 趋势 |
