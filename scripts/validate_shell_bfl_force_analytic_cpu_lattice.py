...
from tensorlbm.d3q19 import W as W19  # noqa: E402
from tensorlbm.d3q19 import equilibrium3d as equilibrium19  # noqa: E402
...
# analytic fields
# --------------------------------------------------------------------------- #
def _equilibrium(rho, ux, uy, uz):
    return equilibrium19(rho, ux, uy, uz) if LATTICE == "D3Q19" else equilibrium27(rho, ux, uy, uz)
...
def uniform_equilibrium(shape, u_in, device):
    ...
    return _equilibrium(rho, ux, z, z).to(torch.float64)
...
def rigid_rotation_equilibrium(shape, centre, omega, device):
    ...
    return _equilibrium(rho, ux, uy, uz).to(torch.float64)
...
def ideal_reflection_force_torque(octree, f_prev, *, wall, weights, origin):
    ...
    mask[0] = False
    c_vec = octree._c_vec.to(torch.float64)
    w = (W19 if LATTICE == "D3Q19" else W27).to(torch.float64)
...
# driver
# --------------------------------------------------------------------------- #
def build_shell(radius, d_max, device, lattice=None):
...
def main():
    ...
    ap.add_argument("--lattice", default="D3Q27", choices=["D3Q19", "D3Q27"])
    args = ap.parse_args()
