...
if [[ $# -lt 1 || $# -gt 3 ]]; then
  echo "usage: $0 PHYSICAL_GPU [RESULT_DIR] [WAIT_FOR_PID]" >&2
  exit 2
fi
...
stem="$result_dir/suboff-nested-v21-aff8-four-level-l90-bounded-allocation-r1"
if [[ -f $stem.json ]]; then
  echo "bounded AFF-8 allocation result already exists: $stem.json" >&2
  exit 0
fi
if [[ -f $stem.ckpt ]]; then
