...
usage() {
  echo "usage: $0 L256|L384|L512 PHYSICAL_GPU [RESULT_DIR] [WAIT_FOR_PID]" >&2
  exit 2
}
...
length=${1#L}
gpu=$2
result_dir=${3:-results/flat_plate_grid_convergence}
wait_for_pid=${4:-}
...
exec "$python" examples/flat_plate_wall_model_validate.py \
  ...
  --reynolds 1000000 --resolved-reynolds 20000 --lattice-speed 0.06 \
