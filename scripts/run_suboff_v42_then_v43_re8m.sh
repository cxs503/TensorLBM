...
if [[ $# != 2 ]]; then
  echo "usage: $0 V42_OWNER_PID PHYSICAL_GPU_UUID" >&2
  exit 2
fi
owner_pid=$1
gpu=$2
...
while [[ -r /proc/$owner_pid/cmdline ]]; do
  ...
  if [[ $command_line != *run_suboff_v41_then_v42_re4m* \
     && $command_line != *suboff-nested-v42-re4m-mixed-fp64compute-l90-3k* ]]; then
    echo "PID $owner_pid no longer belongs to the registered SUBOFF v42 queue" >&2
    exit 3
  fi
  sleep 20
done
...
v42=$result_dir/suboff-nested-v42-re4m-mixed-fp64compute-l90-3k.json
schedule=results/collision-viscosity-natural-kbc-suboff-re8m-mixed-exactweights-r1.json
[[ -f $v42 ]] || { echo "missing SUBOFF v42 result" >&2; exit 4; }
[[ -f $schedule ]] || { echo "missing exact-weight Re8M viscosity schedule" >&2; exit 4; }

"$python" - "$v42" "$schedule" <<'PY'
import json
import math
import sys
...
if not required:
    raise SystemExit("SUBOFF v42 failed the registered Re8M launch health gate")
if schedule.get("schema") != "tensorlbm-collision-viscosity-schedule-v1":
    raise SystemExit("unsupported Re8M viscosity schedule")
if not schedule.get("acceptance", {}).get(
    "all_levels_recover_configured_viscosity"
):
    raise SystemExit("Re8M collision viscosity is not certified")
if float(schedule.get("acceptance", {}).get(
    "minimum_fitted_log_decay", 0.0
)) < 0.004:
    raise SystemExit("Re8M viscosity certificate lacks a resolved decay signal")
if schedule.get("dtype") != "float32" or schedule.get(
    "natural_kbc_compute_dtype"
) != "float64":
    raise SystemExit("Re8M viscosity certificate precision mismatch")
if schedule.get("d3q19_weight_precision_scheme") != (
    "rational_binary64_cast_to_runtime_dtype_v1"
):
    raise SystemExit("Re8M viscosity certificate weight precision mismatch")
expected = [0.500002025, 0.50000405, 0.5000081, 0.5000162]
if len(schedule.get("taus", [])) != len(expected) or any(
    ...
    raise SystemExit("Re8M viscosity certificate tau mismatch")
PY

exec scripts/run_suboff_mixed_precision_re8m_long.sh "$gpu"
