...
# Four physical levels: L=90 -> 180 -> 360 -> 720.  The first allocation probe
# with a 24-cell auxiliary CV consumed 23.6 GiB and was rejected before step
# one.  This revision keeps a 12-cell auxiliary CV plus its streaming-source
# guard away from the interface and uses the measured 1100 B/cell coefficient.
# A one-cell exchange distance is the interpolation
...
export TENSORLBM_DEEP_WALL_MARGIN=${TENSORLBM_DEEP_WALL_MARGIN:-7}
export TENSORLBM_DEEP_WAKE_CELLS=${TENSORLBM_DEEP_WAKE_CELLS:-14}
...
export TENSORLBM_MEMORY_BYTES_PER_CELL=${TENSORLBM_MEMORY_BYTES_PER_CELL:-1100}
