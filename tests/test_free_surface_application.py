...
def test_real_small_config_delegates_to_existing_runner_once(tmp_path, monkeypatch):
    ...
    result = app.run_free_surface_scenario(
        app.FreeSurfaceScenario("small-real", _fs_config(tmp_path))
    )
...
def test_existing_strict_runner_failure_maps_to_failed_result(tmp_path, monkeypatch):
    ...
    result = app.run_free_surface_scenario(
        app.FreeSurfaceScenario("strict-failure", _fs_config(tmp_path))
    )
...
def test_runner_call_boundary_has_no_loop_step_or_backend_compile():
    ...
    function = next(
        ...
        if isinstance(node, ast.FunctionDef) and node.name == "run_free_surface_scenario"
    )
    assert not any(
        isinstance(node, (ast.For, ast.While, ast.AsyncFor)) for node in ast.walk(function)
    )
