...
def test_diagnostic_gate_rejects_underresolved_or_unsettled_runs(module):
    with pytest.raises(ValueError, match="D >="):
        module.validate_suboff_diagnostic(
            nx=192, ny=72, nz=72, hull_length=96, n_steps=600, warmup=300, u_in=0.06
        )
    with pytest.raises(ValueError, match="stern-to-outlet"):
        module.validate_suboff_diagnostic(
            nx=416, ny=208, nz=208, hull_length=206, n_steps=1000, warmup=300, u_in=0.06
        )


def test_diagnostic_gate_accepts_resolved_time_window(module):
    module.validate_suboff_diagnostic(
        nx=416, ny=208, nz=208, hull_length=206, n_steps=3600, warmup=2800, u_in=0.06
    )
