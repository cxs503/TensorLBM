    ...
    configuration.update(
        {
            ...
            "tau_by_level": [0.5 + 16.2 * 2**level / resolved_reynolds for level in range(4)],
            "wall_law": "musker",
            "sponge_inlet": True,
        ...
        json.dumps(
            {
                ...
                "natural_kbc_compute_dtype": configuration["natural_kbc_compute_dtype"],
...
def test_assessor_accepts_matched_causal_sequence_without_physical_claim(tmp_path) -> None:
    ...
    schedules = [
        _schedule(tmp_path / f"schedule-{index}.json", path) for index, path in enumerate(paths)
    ]
    ...
    assert result["component_trends"]["mean_resistance_n"]["trend"] == ("strictly_decreasing")
...
def test_assessor_rejects_uncertified_configured_reynolds(tmp_path) -> None:
    ...
    assert result["acceptance"]["all_collision_viscosity_schedules_admitted"] is False
