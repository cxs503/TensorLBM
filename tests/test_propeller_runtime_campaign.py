...
def test_actual_dynamic_geometry_multi_j_campaign_writes_windowed_samples(tmp_path: Path) -> None:
    ...
    assert [status["j_actual"] for status in summary["campaign"]["per_j_window_status"]] == [
        2 / 3,
        1.0,
        4 / 3,
    ]
    assert all(
        status["convergence"]["window_converged"] is False
        for status in summary["campaign"]["per_j_window_status"]
    )
    for result in summary["results"]:
        ...
        assert (
            result["control_volume_cross_check"]["method"]
            == "discrete_full_control_volume_momentum_budget"
        )
        assert result["control_volume_cross_check"]["available"] is True
        assert result["control_volume_cross_check"]["status"] == "comparable"
        assert (
            result["control_volume_cross_check"]["same_operator_action_reaction_status"]
            == "comparable"
        )
        ...
        assert all(
            sample["wall_action_reaction_signed_residual_norm"] == pytest.approx(0.0, abs=5e-6)
            for sample in result["samples"]
        )
        assert all(
            sample["wall_action_reaction_absolute_residual_norm"] == pytest.approx(0.0, abs=5e-6)
            for sample in result["samples"]
        )
        assert all(
            sample["wall_action_reaction_relative_residual"] < 1e-5 for sample in result["samples"]
        )
        assert all(
            sample["wall_momentum_contribution_x"] + sample["wall_reaction_x"]
            == pytest.approx(0.0, abs=5e-6)
            for sample in result["samples"]
        )
        ...
        assert all(
            sample["wall_torque_action_reaction_signed_residual_norm"]
            == pytest.approx(0.0, abs=5e-6)
            for sample in result["samples"]
        )
        assert all(
            sample["wall_torque_action_reaction_absolute_residual_norm"]
            == pytest.approx(0.0, abs=5e-6)
            for sample in result["samples"]
        )
        assert all(
            sample["wall_torque_action_reaction_relative_residual"] < 1e-5
            for sample in result["samples"]
        )
        assert all(
            sample["wall_fluid_torque_impulse_x"] + sample["wall_reaction_torque_x"]
            == pytest.approx(0.0, abs=5e-6)
            for sample in result["samples"]
        )
...
def test_actual_three_level_true_spatial_refinement_campaign_is_fail_closed(tmp_path: Path) -> None:
    ...
    evidence = run_propeller_resolution_sensitivity(
        (coarse, medium, fine), level_names=("coarse", "medium", "fine")
    )
    ...
    assert (
        basis["low_mach_matched"]
        is basis["advance_ratios_matched"]
        is basis["re_d_matched"]
        is True
    )
    ...
    assert [level["domain_per_diameter"] for level in basis["levels"]] == [
        [400.0, 200.0, 200.0]
    ] * 3
...
def test_resolution_sensitivity_rejects_rounded_rpm_angular_claim_before_run(
    tmp_path: Path,
) -> None:
    ...
    rounded_candidate = PropellerBenchmarkConfig(**{**base.__dict__, "rpm": 1.0 / 200.4})
...
def test_resolution_sensitivity_requires_whole_rotation_windows(tmp_path: Path) -> None:
    ...
    partial_window = PropellerBenchmarkConfig(**{**base.__dict__, "sample_window_steps": 100})
