...
def _event(**changes: object) -> IToGPopulationTransferEvent:
    values: dict[str, object] = {
        "event_id": "step-1/i-to-g-0",
        "lattice": D3Q19,
        "converting_donors": ((1, 1, 1),),
        "source_partition": ((1, 1, 1),),
        "destination_partition": ((1, 1, 2),),
        "source_phase_before": Phase.I,
        "source_phase_after": Phase.G,
        "destination_phase_before": Phase.I,
...
def _evidence(dtype: torch.dtype = torch.float64, **changes: object) -> PopulationTransferEvidence:
    ...
    values: dict[str, object] = {
        ...
        "independent_mass": IndependentMassLedger(
            -torch.tensor(0.25, dtype=dtype), torch.tensor(0.25, dtype=dtype), zero
        ),
    }
    values.update(changes)
...
def test_direct_donor_copy_with_donor_zeroing_has_nonzero_population_residual() -> None:
    ...
    report = validate_i_to_g_population_transfer(
        ...
        _evidence(
            donor_before=donor_before,
            donor_after=torch.zeros_like(donor_before),
            receiver_after=donor_before.clone(),
        ),
    )
...
def test_unpaired_copy_without_donor_debit_is_rejected_by_population_sum() -> None:
    ...
    report = validate_i_to_g_population_transfer(
        _event(),
        _evidence(
            donor_before=donor_before, donor_after=donor_before, receiver_after=donor_before.clone()
        ),
    )
...
def test_source_destination_partitions_cannot_overlap_or_infer_non_i_receiver() -> None:
    overlap = validate_i_to_g_population_transfer(
        _event(destination_partition=((1, 1, 1),)), _evidence()
    )
    bad_owner = validate_i_to_g_population_transfer(
        _event(destination_phase_before=Phase.G), _evidence()
    )
