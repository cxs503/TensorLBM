...
def test_single_speed_persists_each_post_warmup_me_sample(
    ...
    monkeypatch.setattr(
        propeller_benchmark, "make_channel_wall_mask_3d", lambda *args, **kwargs: mask
    )
    monkeypatch.setattr(
        propeller_benchmark,
    ...
    monkeypatch.setattr(
        propeller_benchmark, "equilibrium3d", lambda *args, **kwargs: torch.zeros((19, 20, 20, 40))
    )
    ...
    monkeypatch.setattr(
        propeller_benchmark, "apply_zou_he_channel_boundaries_3d", lambda f, **kwargs: f
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *args: (torch.tensor(next(values)), torch.tensor(0.0), torch.tensor(0.0)),
    )
    monkeypatch.setattr(
        propeller_benchmark,
