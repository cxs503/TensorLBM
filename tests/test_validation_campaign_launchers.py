...
def test_nested_v4_launcher_expands_audited_l150_continuation(
    ...
    assert arguments[arguments.index("--reflux-correction-stencil") + 1] == ("exterior_cells")
    ...
    assert arguments[arguments.index("--stress-exchange-distance") + 1] == ("7.03125")
    ...
    assert arguments[arguments.index("--maximum-reflux-applied-correction-fraction") + 1] == "0.001"
    assert arguments[arguments.index("--cs-smag") + 1] == "0.05"
    assert arguments[arguments.index("--collision-model") + 1] == ("cumulant_smagorinsky")
    assert arguments[arguments.index("--wale-cw") + 1] == "0.5"
    assert arguments[arguments.index("--vreman-cv") + 1] == "0.025"
    assert arguments[arguments.index("--minimum-target-reynolds-convective-times") + 1] == "7.5"
...
def test_mass_conservative_l90_launcher_can_form_clean_inlet_sponge_pair(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_v13_mass_conservative_l90.sh"),
            "L90",
            "0",
            str(tmp_path),
        ],
    ...
    arguments = completed.stdout.splitlines()
    assert arguments[arguments.index("--collision-model") + 1] == "natural_kbc"
    assert arguments[arguments.index("--stress-exchange-distance") + 1] == ("4.21875")
...
def test_l120_multigpu_probe_preserves_geometric_similarity_and_memory_gates(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_v20_l120_multigpu_allocation_probe.sh"),
            "GPU-a,GPU-b,GPU-c",
            str(tmp_path),
        ],
        cwd=ROOT,
    ...
    assert completed.returncode == 0, completed.stderr
    arguments = completed.stdout.splitlines()
    assert arguments[arguments.index("--level-devices") + 1] == ("cuda:0,cuda:0,cuda:1,cuda:2")
...
def test_aff8_bounded_probe_keeps_exact_geometry_and_fail_closed_memory(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_v21_aff8_bounded_allocation_probe.sh"),
            "GPU-a",
            str(tmp_path),
        ],
        cwd=ROOT,
...
def test_flat_plate_v5_uses_a_distinct_checkpoint_generation(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_flat_plate_v5_mass_conservative_level.sh"),
            "L256",
            "0",
            str(tmp_path),
        ],
...
def test_suboff_v9_launcher_uses_quadratic_inlet_pressure_observer(
    ...
    assert arguments[arguments.index("--surface-pressure-extrapolation") + 1] == "quadratic"
    output = arguments[arguments.index("--output") + 1]
...
def test_suboff_v9_launcher_can_extend_only_time_and_statistics(
    ...
    assert arguments[arguments.index("--statistics-window-steps") + 1] == ("26000")
    output = arguments[arguments.index("--output") + 1]
    checkpoint = arguments[arguments.index("--checkpoint") + 1]
...
def test_sphere_launcher_names_natural_kbc_variant_separately(
    ...
    assert completed.returncode == 0, completed.stderr
    arguments = completed.stdout.splitlines()
    assert arguments[arguments.index("--collision-model") + 1] == ("natural_kbc_d3q19")
    output = arguments[arguments.index("--output") + 1]
...
def test_gradient_sgs_pilot_launcher_records_vreman_coefficient(
    ...
    assert completed.returncode == 0, completed.stderr
    arguments = completed.stdout.splitlines()
    assert arguments[arguments.index("--collision-model") + 1] == ("cumulant_vreman")
...
def test_cylinder_r18_extends_the_same_scaled_grid_family(
    ...
    assert arguments[arguments.index("--statistics-window-steps") + 1] == ("45000")
...
def test_cylinder_v8_changes_only_registered_streamwise_clearance(
    ...
    assert arguments[arguments.index("--radius") + 1] == "9"
    assert arguments[arguments.index("--center-x-fraction") + 1] == ("0.3333333333333333")
    assert arguments[arguments.index("--collision-model") + 1] == ("planar_cumulant_d2q9")
...
def test_nested_v10_launcher_scales_all_inner_physical_locations(
    ...
    assert arguments[arguments.index("--stress-exchange-distance") + 1] == ("3.515625")
...
def test_v22_compiled_allocation_probe_uses_tensor_tau_executor(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_v22_compiled_allocation_probe.sh"),
            "L90",
            "0",
            str(tmp_path),
        ],
...
def test_v28_allocation_probe_uses_scaled_wall_exchange(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_v28_scaled_wall_allocation_probe.sh"),
            "L90",
            "0",
            str(tmp_path),
        ],
    ...
    arguments = completed.stdout.splitlines()
    assert "--compile-natural-kbc" in arguments
    assert arguments[arguments.index("--stress-exchange-distance") + 1] == ("8.4375")
    output = arguments[arguments.index("--output") + 1]
...
def test_compiled_production_launchers_lock_corrected_boundary_and_memory_path(
    ...
    assert arguments[arguments.index("--stress-exchange-distance") + 1] == (expected_exchange)
    output = arguments[arguments.index("--output") + 1]
...
def test_generic_compiled_reynolds_pilot_records_generation_and_target(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_compiled_reynolds_pilot.sh"),
            "v25",
            "500000",
            "L90",
            "0",
...
def test_v29_re200k_pilot_changes_only_scaled_wall_location(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_suboff_nested_v29_re200k_scaled_wall_l90.sh"),
            "L90",
            "0",
            str(tmp_path),
        ],
...
def test_v30_re200k_long_run_continues_v23_with_suboff_exchange_family(
    ...
    assert arguments[arguments.index("--stress-exchange-distance") + 1] == "1.0"
    assert (
        arguments[arguments.index("--wall-exchange-distance-over-length-target") + 1]
...
def test_sphere_v4_launcher_locks_bounded_compiled_family(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_sphere_v4_bounded_natural_kbc_level.sh"),
            "R15",
            "0",
            str(tmp_path),
        ],
...
def test_sphere_v5_w20_r9_is_memory_bounded_domain_pilot(
    tmp_path: Path,
) -> None:
    fake_python = tmp_path / "python"
    fake_python.write_text(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_sphere_v5_domain_w20_r9.sh"),
            "0",
            str(tmp_path),
        ],
        cwd=ROOT,
    ...
    arguments = completed.stdout.splitlines()
    assert arguments[arguments.index("--radius") + 1] == "9"
    assert arguments[arguments.index("--nx") + 1] == "216"
    assert arguments[arguments.index("--ny") + 1] == "180"
    assert arguments[arguments.index("--nz") + 1] == "180"
    output = arguments[arguments.index("--output") + 1]
    assert "sphere-v5-natural-kbc-domain-w20-r9-7200" in output


def test_sphere_v5_w24_r9_is_third_domain_member(
    tmp_path: Path,
) -> None:
    fake_python = tmp_path / "python"
    fake_python.write_text(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_sphere_v5_domain_w24_r9.sh"),
            "0",
            str(tmp_path),
        ],
        cwd=ROOT,
    ...
    assert arguments[arguments.index("--radius") + 1] == "9"
    assert arguments[arguments.index("--nx") + 1] == "216"
    assert arguments[arguments.index("--ny") + 1] == "216"
    assert arguments[arguments.index("--nz") + 1] == "216"
    output = arguments[arguments.index("--output") + 1]
    assert "sphere-v5-natural-kbc-domain-w24-r9-7200" in output
...
def test_sphere_v6_r9_is_matched_no_inlet_sponge_ab(
    ...
    assert arguments[arguments.index("--far-field-mode") + 1] == ("non_equilibrium_extrapolation")
    output = arguments[arguments.index("--output") + 1]
...
def test_sphere_v8_is_no_inlet_w24_factorial_member(
    tmp_path: Path,
) -> None:
    fake_python = tmp_path / "python"
    fake_python.write_text(
    ...
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_sphere_v8_no_inlet_domain_w24_r9.sh"),
            "0",
            str(tmp_path),
        ],
        cwd=ROOT,
    ...
    assert "--sponge-inlet" not in arguments
    output = arguments[arguments.index("--output") + 1]
    assert "sphere-v8-natural-kbc-no-inlet-domain-w24-r9-7200" in output
...
def test_sphere_v4_convergence_assessor_uses_only_matching_family(
    ...
    for radius, steps in ((9, 7200), (12, 9600), (15, 12000)):
        (tmp_path / (f"sphere-v4-natural-kbc-equivalent-r{radius}-{steps}.json")).write_text(
            "{}\n", encoding="utf-8"
        )
    env = os.environ.copy()
    env["TENSORLBM_PYTHON"] = str(fake_python)
...
def test_cylinder_r18_assessor_extends_matching_grid_family(
    ...
    for radius, steps in ((9, 54000), (12, 72000), (15, 90000), (18, 108000)):
        (tmp_path / (f"cylinder-v4-equivalent-r{radius}-{steps}.json")).write_text(
            "{}\n", encoding="utf-8"
        )
    env = os.environ.copy()
    env["TENSORLBM_PYTHON"] = str(fake_python)
    completed = subprocess.run(
        [
            "bash",
            str(ROOT / "scripts" / "run_cylinder_v4_r18_convergence_assess.sh"),
            str(tmp_path),
        ],
        cwd=ROOT,
        env=env,
