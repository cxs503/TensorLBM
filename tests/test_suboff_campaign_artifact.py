...
def _save_checkpoint(
    directory: Path, step: int, rank: int, *, friction: float, pressure: float, samples: int
) -> None:
    directory.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "metadata": _metadata(rank),
            "step": step,
...
def _short_completed_campaign(root: Path) -> dict[str, object]:
    for rank in range(2):
        ...
        _save_checkpoint(root / "step-2", 2, rank, friction=10.0, pressure=20.0, samples=2)
        _save_checkpoint(root / "step-4", 4, rank, friction=16.0, pressure=28.0, samples=4)
    return {
        "schema": "suboff-d3q27-segmented-run-v1",
        "checkpoint_metadata": _metadata(0),
        "expected_ranks": [0, 1],
...
def test_withheld_short_run_builds_and_revalidates_a_hash_bound_artifact(tmp_path):
    ...
    assert all(record["population_shape"] == [27, 1, 2, 2] for record in artifact["checkpoints"])
...
def test_step_200_short_run_without_post_gate_blocks_is_explicitly_withheld(tmp_path):
    for rank in range(2):
        _save_checkpoint(
            tmp_path / "step-200", 200, rank, friction=123.0, pressure=456.0, samples=200
        )
    manifest = {
        "schema": "suboff-d3q27-segmented-run-v1",
        "checkpoint_metadata": _metadata(0),
        "expected_ranks": [0, 1],
        "segments": [{"start_step": 0, "end_step": 200, "checkpoint": "step-200"}],
        "blocks": [],
    }
...
def test_post_gate_blocks_require_finite_positive_dynamic_pressure_wetted_area_and_emit_ct(
    tmp_path,
):
    manifest = _short_completed_campaign(tmp_path)
...
def test_blocks_must_be_strictly_ordered_nonoverlapping_and_use_each_boundary_once(
    tmp_path, second
):
    manifest = _short_completed_campaign(tmp_path)
...
@pytest.mark.parametrize(
    "population",
    [torch.ones((27, 1, 2, 2), dtype=torch.int64), torch.full((27, 1, 2, 2), float("nan"))],
)
