...
def test_i_to_g_ownership_transaction_rejects_tampered_debit_credit_evidence() -> None:
    ...
    tampered = replace(
        transaction, receiver_credit=transaction.receiver_credit + torch.tensor(0.125)
    )
...
def test_i_to_g_ownership_transaction_rejects_tampered_receiver_increment() -> None:
    ...
    with pytest.raises(TopologyTransactionError, match="WITHHELD.*receiver credit aggregation"):
        replace(
            transaction, receiver_increment=torch.zeros_like(transaction.receiver_increment)
        ).validate()
...
def test_topology_transaction_rejects_i_to_g_increment_detached_from_ownership_evidence() -> None:
    ...
    with pytest.raises(TopologyTransactionError, match="WITHHELD.*tampered I→G increment evidence"):
        build_topology_transaction(
            ...
            i_to_g_increment=torch.zeros_like(mass),
            i_to_g_ownership=transaction,
            rho_liquid=1.0,
            rho_gas=0.001,
            solid_mask=solid,
            gas_flag=GAS,
...
def test_i_to_g_mixed_legacy_increment_rejects_overflow_before_clamp() -> None:
    ...
    with pytest.raises(TopologyTransactionError, match="WITHHELD.*combined receiver capacity"):
        build_topology_transaction(
            ...
            to_gas=donor,
            recv_new=zero,
            redistribution_increment=legacy,
            i_to_g_increment=transaction.receiver_increment,
            i_to_g_ownership=transaction,
            rho_liquid=1.0,
            rho_gas=0.001,
            solid_mask=solid,
...
def test_i_to_g_multi_donor_same_receiver_uses_combined_capacity_and_exact_records() -> None:
    ...
    transaction = build_i_to_g_ownership_transaction(
        ...
        mass,
        to_gas=donors,
        to_liq=torch.zeros_like(donors),
        solid_mask=solid,
        gas_flag=GAS,
        liquid_flag=LIQUID,
        interface_flag=INTERFACE,
...
def test_free_surface_step_i_to_g_failure_is_withheld_and_leaves_state_and_ledgers_unchanged(
    ...
    with pytest.raises(
        TopologyTransactionError, match="WITHHELD.*entire free_surface_step topology candidate"
    ):
        free_surface_step(
            f,
            fill,
            ...
            conversion_density_audit_ledger=audit,
...
def test_default_step_does_not_construct_i_to_g_proposal_and_matches_explicit_disabled(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("default must not build I→G proposal")
        ),
    )
