...
def _jonswap_spectrum(
    omega: torch.Tensor, omega_peak: float = 1.0, gamma: float = 3.3
) -> torch.Tensor:
    ...
    return (
        ...
        * gravity**2
        * positive_omega**-5
        * torch.exp(-1.25 * (omega_peak / positive_omega) ** 4)
        * gamma**peak
    )
...
def test_hydrodynamic_rao_and_natural_frequency_are_finite() -> None:
    ...
    rao = compute_rao_from_hydrodynamics(
        radiation.omega,
        radiation.added_mass,
        radiation.damping,
        hydro.c_matrix,
        body.build_mass_matrix(),
        excitation,
    )
    assert rao.rao_amplitude.shape == (32, 6)
    assert torch.isfinite(rao.rao_amplitude).all()
    natural = compute_natural_frequencies(
        body.build_mass_matrix(), radiation.added_mass[0], hydro.c_matrix
    )
...
def test_irregular_wave_generation_is_seed_reproducible_and_sra_is_positive() -> None:
    ...
    first = generate_jonswap_excitation(
        radiation.omega, spectrum, unit_transfer, 10.0, 16, 20.0, 0.1, seed=12
    )
    second = generate_jonswap_excitation(
        radiation.omega, spectrum, unit_transfer, 10.0, 16, 20.0, 0.1, seed=12
    )
