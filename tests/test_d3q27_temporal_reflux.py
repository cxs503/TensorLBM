...
def test_uniform_equilibrium_has_zero_temporal_mismatch_and_corrections() -> None:
    ...
    assert all(
        torch.equal(correction, torch.zeros_like(coarse.flux))
        for correction in result.fine_corrections
    )
...
def test_arbitrary_packets_are_conservative_in_mass_and_all_momentum_components() -> None:
    ...
    fine_mass, fine_momentum = _moments(
        result.corrected_fine_fluxes[0] + result.corrected_fine_fluxes[1]
    )
    ...
    assert torch.allclose(
        mismatch_momentum, raw_coarse_momentum - raw_fine_momentum, rtol=0.0, atol=1e-14
    )
    ...
    assert torch.equal(
        result.coarse_correction + sum(result.fine_corrections), torch.zeros_like(coarse_flux)
    )
