...
for d in range(1, 19):
    ...
    if fluid_bdry.any():
        ...
        print(f"Neighbor: ({i+dcx}, {j+dcy}, {k+dcz}) solid={bool(solid[k+dcz, j+dcy, i+dcx].item())}")
        ...
        # Fluid cell
        d_fluid = _suboff_surface_distance(float(i), float(j), float(k), cx, cy, cz, hull_length, config)
        ...
        # Neighbor cell
        d_nb = _suboff_surface_distance(float(i+dcx), float(j+dcy), float(k+dcz), cx, cy, cz, hull_length, config)
        ...
        print(f"  Neighbor surface radius at xi={xi_n:.6f}: r_surf={r_surf_needed:.6f}, actual r={r_n:.6f}")
