...
from tensorlbm.suboff_full_wet_force_window_campaign import (
...
def _inputs(capture_steps: tuple[int, ...] = (1, 2)) -> tuple[GeometryAsset, FullyWettedFlowConfig]:
    ...
    config = FullyWettedFlowConfig(
        ...
        inlet_velocity=0.03,
        steps=2,
        capture_population_steps=capture_steps,
    )
    return asset, config
...
def test_campaign_measures_two_real_full_wet_snapshots_deterministically() -> None:
    ...
    assert first["sample_windows"]["mean_force"] == pytest.approx(
        first["force_window"]["observation"]["force"]
    )
    ...
    assert all(
        record["sample_phase"] == "post_stream_pre_bounce_back" for record in first["force_records"]
    )
    assert all(
        record["population_source"] == "full_wet_opt_in_production_snapshot"
        for record in first["force_records"]
    )
