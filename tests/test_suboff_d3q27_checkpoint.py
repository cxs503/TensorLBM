...
def test_restart_matches_uninterrupted_and_continues_force_accumulators(tmp_path, module):
    ...
    initial = equilibrium27(
        rho, torch.zeros_like(rho), torch.zeros_like(rho), torch.zeros_like(rho)
    )
    ...
    metadata = module.suboff_checkpoint_metadata(
        nx=nx, ny=ny, nz=nz, hull_length=24.0, re=2000.0, u_in=0.03, y_val=0.5, world_size=1, rank=0
    )
    ...
    split[..., 0] = 12345.0
    split[..., -1] = -54321.0
    resumed, state = module.load_suboff_checkpoint(
        tmp_path / "suboff",
        metadata=metadata,
        shape=split.shape,
        rank=0,
    ...
    assert (state["step"], state["friction_sum"], state["pressure_sum"], state["drag_samples"]) == (
        2,
        1.25,
        2.5,
        2,
...
def test_restart_rejects_incompatible_static_metadata(tmp_path, module):
    ...
    metadata = module.suboff_checkpoint_metadata(
        nx=4, ny=1, nz=1, hull_length=24.0, re=2000.0, u_in=0.03, y_val=0.5, world_size=1, rank=0
    )
    module.save_suboff_checkpoint(
        tmp_path / "suboff",
        f=f,
