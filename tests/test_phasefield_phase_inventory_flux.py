...
def test_periodic_inventory_flux_uses_actual_step_states_and_only_claims_zero_net_stream_transfer(
    monkeypatch,
):
    ...
    initial = initialize_free_energy_collision_only_state(
...
def test_no_flux_reports_structurally_zero_adapter_crossing_without_inferring_collision_or_physical_flux(
    monkeypatch,
):
    _identity_collision(monkeypatch)
    f = torch.zeros((19, 2, 2, 3), dtype=torch.float64)
    ...
    assert all(
        sample.stream_boundary_outgoing_g == pytest.approx(0.0) for sample in diagnostic.steps[1:]
    )
    assert all(
        sample.stream_boundary_incoming_g == pytest.approx(0.0) for sample in diagnostic.steps[1:]
    )
    assert all(
        sample.stream_boundary_net_g == pytest.approx(0.0) for sample in diagnostic.steps[1:]
    )
    assert all(
        sample.stream_boundary_crossing_status == "no_flux_reflection_zero_crossing"
        for sample in diagnostic.steps[1:]
    )
    assert diagnostic.collision_contribution is None
    assert diagnostic.physical_phase_flux is None
    assert diagnostic.physical is False
