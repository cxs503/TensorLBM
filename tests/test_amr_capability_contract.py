...
def test_matrix_distinguishes_mechanics_from_frontend_ready_physics() -> None:
    ...
    assert (
        matrix["adaptive_dynamic"]["D2Q9"]["single_phase"].mechanics_status
    ...
    assert (
        matrix["surface_shell"]["D3Q19"]["single_phase"].mechanics_status
        == "AVAILABLE_MECHANICS_ONLY"
    )
    ...
    assert (
        matrix["adaptive_dynamic"]["D3Q27"]["single_phase"].status
        == "WITHHELD_NO_D3Q27_LOCAL_REFINEMENT"
    )
    assert all(
...
def test_required_frontend_metadata_is_explicit_and_current_paths_fail_closed() -> None:
    ...
    with pytest.raises(
        LocalRefinementWithheldError, match="WITHHELD_REQUIRED_METADATA_NOT_EMITTED"
    ):
        require_local_refinement_capability(
            "adaptive_dynamic",
            "D3Q19",
...
def test_contract_rejects_unknown_or_under_evidenced_combinations() -> None:
    with pytest.raises(
        LocalRefinementWithheldError, match="WITHHELD_NO_COUPLED_AMR_PHYSICS_CONTRACT"
    ):
...
def test_public_contract_fail_closes_unknown_inputs_before_matrix_lookup(
    ...
    with pytest.raises(LocalRefinementWithheldError, match=withheld_code):
        require_local_refinement_capability(
            ...
            lattice,
            physics,
            metadata={"ratio": 2},  # type: ignore[arg-type]
        )
