...
def test_periodic_central_operators_are_second_order_stencils():
    ...
    expected_lap = (
        sum(torch.roll(field, shift, dim) for dim in range(3) for shift in (-1, 1)) - 6 * field
    )
...
def test_free_energy_step_matches_pre_common_operator_d3q19_formula():
    ...
    phi = g.sum(dim=0)
    kwargs = dict(
        tau_f=0.9, tau_g=0.8, A=0.11, B=0.13, kappa=0.02, Gamma=0.4, gx=0.001, gy=-0.002, gz=0.003
    )
    ...
    cu = (
        cv[:, 0].view(19, 1, 1, 1) * ux
        + cv[:, 1].view(19, 1, 1, 1) * uy
        + cv[:, 2].view(19, 1, 1, 1) * uz
    )
    u_sq = (ux**2 + uy**2 + uz**2).unsqueeze(0)
    expected_g_eq = w * phi.unsqueeze(0) * (1.0 + 3.0 * cu + 4.5 * cu**2 - 1.5 * u_sq) + w * kwargs[
        "Gamma"
    ] * (csq / (1.0 / 3.0) - 3.0) * mu.unsqueeze(0)
