...
def test_planner_generates_exact_gap_free_segments_and_post_gate_blocks():
    ...
    assert manifest["blocks"] == [
        {
            "first_sample_step": 1_501,
            "last_sample_step": 2_000,
            "friction_sum": None,
            "pressure_sum": None,
            "drag_samples": 500,
        },
        {
            "first_sample_step": 2_001,
            "last_sample_step": 3_000,
            "friction_sum": None,
            "pressure_sum": None,
            "drag_samples": 1_000,
        },
        {
            "first_sample_step": 3_001,
            "last_sample_step": 4_000,
            "friction_sum": None,
            "pressure_sum": None,
            "drag_samples": 1_000,
        },
...
def test_planner_can_start_ct_later_but_rejects_ct_before_outlet_convection():
    ...
    manifest = build_suboff_segmented_campaign_manifest(
        config, segment_steps=1_000, segment_count=3
    )
...
def test_planner_rejects_incomplete_outlet_transient_and_insufficient_campaign():
    ...
    with pytest.raises(ValueError, match="no wholly post-gate Ct samples"):
        build_suboff_segmented_campaign_manifest(
            _static_config(), segment_steps=500, segment_count=3
        )
