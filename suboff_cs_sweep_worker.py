#!/usr/bin/env python3
"""SUBOFF bare-hull Smagorinsky Cs sweep — Re=1000.

Runs MRT+Smagorinsky on SUBOFF bare hull with a user-specified Smagorinsky
constant (Cs) and writes results JSON.  Launched in parallel (one per SDAA
card) to compare different Cs values.

Parameters (fixed across all Cs):
  ...
  Normals: SurfaceMesh.from_gradient (generic gradient-based)

Usage:
  python suboff_cs_sweep_worker.py <Cs> <device_id> <output_path>
  Cs: e.g. 0.01 | 0.05 | 0.10 | 0.15
"""
import json
import math
import sys
...
def run(cs_smag, device_id, output_path=None):
    tag = f"[SDAA:{device_id} Cs={cs_smag}]"
    ...
    for step in range(1, n_steps + 1):
        ...
        # 2. Collision (MRT + Smagorinsky LES with given Cs)
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python suboff_cs_sweep_worker.py <Cs> <device_id> [output_path]")
        print("  Cs: e.g. 0.01 | 0.05 | 0.10 | 0.15")
        sys.exit(1)

    cs_smag = float(sys.argv[1])
    device_id = int(sys.argv[2])
    output_path = sys.argv[3] if len(sys.argv) > 3 else None

    run(cs_smag, device_id, output_path)
