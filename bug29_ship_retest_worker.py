#!/usr/bin/env python3
"""STL ship hull retest worker — Bug 29 fix + Wigley + Series 60.
...
Tests (SDAA 20-23):
  ...
  TEST 3 (SDAA:22): Wigley analytical hull, Re=1000, L=80, nx=200,ny=80,nz=80,
                    from_gradient normals. Reference: experimental Cd.
  TEST 4 (SDAA:23): Series 60 Cb=0.60, Re=1000, MRT+Smag.
                    Compare from_gradient vs from_stl normals.

Bug 29 fix (already in SurfaceMesh_from_stl):
  - STL normals now use centroid-direction check.
  - If normal points toward solid centroid, flip it.
  - Previously: Cd_p+Cd_f cancelled (normals pointed inward).

Usage:
  PYTHONPATH=src python bug29_ship_retest_worker.py <test_id> <device_id> <output_path>
...
# -----------------------------------------------------------------------
# Core LBM ship benchmark runner
# -----------------------------------------------------------------------
...
def run_ship_lbm(
    ...
    """Run the LBM simulation loop and return drag coefficients."""
    ...
    print(f"{tag} STL ship retest (Bug 29 fix)", flush=True)
    ...
    # Check that normals point outward (away from solid centroid)
    solid_cpu = solid.cpu()
    ...
    if len(solid_coords) > 0:
        ...
        for s in sample:
            ...
            # Outward = normal points away from center = dot < 0
            if dot < 0:
                outward_count += 1
    ...
    near = near.to(device)
    # Move mesh tensors to device (mesh may have been built on CPU)
    ...
    result = {
        "benchmark": f"bug29_retest_{ship_name}",
        "test_id": test_id,
...
def run_test3_wigley(device_id, output_path=None):
    ...
    # For Wigley: S ≈ 2 * integral of half-beam dx dz
    # Use voxel-based estimate: count near-wall cells
    ...
    # Reference Cd for Wigley at Re=1000 (experimental, low Re)
    # Wigley hull Cd ≈ 0.01-0.02 at Re~1000 (form drag dominant)
    cd_ref_wigley = 0.015
...
def run_test4_series60(device_id, output_path=None):
    ...
    # The analytical hull is at cx=nx*0.30, so STL origin must match
    # STL coordinates: x ∈ [0, L], y ∈ [-B/2, B/2], z ∈ [0, T]
    # Grid: cell (ix,iy,iz) center at (ix+0.5, iy+0.5, iz+0.5)
    # We want STL x=0 → ix = cx - L/2, STL x=L → ix = cx + L/2
    # So origin_x = 0 - (cx - L/2) = -(cx - L/2)
    origin_x = -(cx - L / 2.0)
    ...
    # ---- Comparison ----
    comparison = {
        "benchmark": "series60_normal_comparison",
        "test_id": 4,
        "device": f"sdaa:{device_id}",
        "Re": Re,
        "L": L,
