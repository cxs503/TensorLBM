#!/usr/bin/env python3
"""Bounce-back + momentum exchange (Ladd 1994) benchmark runner — V3.
...
V3 CHANGE: Correct Ladd drag formula:
  - f_opp read from SOLID neighbor (not same fluid cell)
  - NO equilibrium subtraction (full Ladd formula)
  - Force on wall = -(f_i + f_opp) * c_i (NEGATED)
  - Check neighbor at -c_i direction (shift solid by +c_i)
Verified: Cylinder Re=200 Cd=1.19 (8.6% error).

Usage:
    PYTHONPATH=src python rettest_v3_worker.py <case_name> <device_id> <output_path>
"""
import json
import math
import sys
...
def drag_ladd(f, near, solid, dpS):
    """Full Ladd (1994) momentum-exchange drag — CORRECT formula.

    Key differences from the broken drag_neq:
    1. f_opp read from SOLID neighbor (not same fluid cell)
    2. NO equilibrium subtraction (full Ladd formula)
    3. Force on wall = -(f_i + f_opp) * c_i  (NEGATED for wall force)
    4. Check neighbor at -c_i direction (shift solid by +c_i)
    ...
    for i in range(1, 19):
        ...
        # Neighbor at -c_i: shift solid by +c_i
...
def run_case(case_name, device_id, output_path):
    ...
    for step in range(1, n_steps + 1):
        ...
        # 6. Compute drag (full Ladd ME, post-stream post-BC)
    ...
    result = {
        ...
        "boundary": "bounce_back+ME(Ladd1994,full)+farfield",
        "grid": f"{nx}x{ny}x{nz}",
        "Re": re,
        "nu": nu,
        "tau": tau,
