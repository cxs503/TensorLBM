"""Quick comparison test of ME variants on D=24 cylinder."""
import sys
...
def me_bfl_after_stream(f_post_bfl, fbm, dpS):
    """ME using post-stream values: F = (f[d](x_f) post-stream + f_bc) * c_i"""
    fx = torch.tensor(0.0, device=device)
    for d in range(1, 19):
        opp_d = int(opp[d].item())
        mask = fbm[d]
        if not mask.any(): continue
        f_i = f_post_bfl[d][mask]  # post-stream at x_f (from x_b)
        f_bc = f_post_bfl[opp_d][mask]  # BFL-reconstructed
        c_d_x = float(c[d, 0].item())
        fx = fx + ((f_i + f_bc) * c_d_x).sum()
    return float(fx.item() / dpS)

def me_bfl_wall_interp(f_prev, fbm, qf, dpS):
    """Wall-surface ME: f_i[wall] = (1-q)*fp_d + q*feq[d](x_s), F = 2*f_i[wall]*c_i"""
    fx = torch.tensor(0.0, device=device)
    for d in range(1, 19):
        opp_d = int(opp[d].item())
        mask = fbm[d]
        if not mask.any(): continue
        q_cell = qf[d][mask]
        fp_d = f_prev[d][mask]  # post-collision at x_f
...
cd_pf = []; cd_me_std = []; cd_me_after = []; cd_me_wall = []
...
for step in range(1, n_steps+1):
    ...
    cd_after = me_bfl_after_stream(f_bfl, fbm, dpS)
    cd_wall = me_bfl_wall_interp(f_post_coll, fbm, qf, dpS)
    
    # Standard BB after stream
    ...
    if step > warmup:
        ...
        if math.isfinite(cd_after): cd_me_after.append(cd_after)
    ...
    if step % 50 == 0:
        print(f'step={step} PF={sum(cd_pf)/max(len(cd_pf),1):.4f} '
              f'ME_std={sum(cd_me_std)/max(len(cd_me_std),1):.4f} '
              f'ME_after={sum(cd_me_after)/max(len(cd_me_after),1):.4f} '
              f'ME_wall={sum(cd_me_wall)/max(len(cd_me_wall),1):.4f}', flush=True)

print(f'FINAL (ref=1.30):')
print(f'  BB+PF        = {sum(cd_pf)/max(len(cd_pf),1):.4f}')
print(f'  BB+ME_std    = {sum(cd_me_std)/max(len(cd_me_std),1):.4f}')
print(f'  BFL+ME_after = {sum(cd_me_after)/max(len(cd_me_after),1):.4f}')
print(f'  BFL+ME_wall  = {sum(cd_me_wall)/max(len(cd_me_wall),1):.4f}')
