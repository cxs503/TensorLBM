...
def run(device_id=0, nx=128, ny=64, nz=4, R=10.0, u_in=0.1, tau=0.53,
        n_steps=5000, warmup=2000):
    ...
    for step in range(1, n_steps + 1):
        ...
        if step > warmup:
            cdp_x, cdp_y, _ = drag_pressure_integration(f, mesh, dpS, solid=solid)
        ...
        if step % 500 == 0 or step == n_steps:
            n = len(cd_p_hist)
            if n > 0:
                cd_p = sum(cd_p_hist[-min(200,n):]) / min(200,n)
                cd_f = sum(cd_f_hist[-min(200,n):]) / min(200,n)
                cd = cd_p + cd_f
                cl = sum(cl_hist[-min(200,n):]) / min(200,n)
                print(f" {step:5d}  {cd_p:8.4f}  {cd_f:8.4f}  {cd:8.4f}  {cl:8.4f}")
            else:
                _, ux, _, _ = macroscopic3d(f)
                ms = float(ux.abs().max().item())
                print(f" {step:5d}  {'---':>8s}  {'---':>8s}  {'---':>8s}  (max|u|={ms:.4f})")
    ...
    if n > 10:
        ...
        # Strouhal from Cl zero crossings
        st = 0.0
        if len(cl_hist) > 20:
            crossings = sum(1 for i in range(1, len(cl_hist))
                           if cl_hist[i-1] < 0 and cl_hist[i] >= 0)
            if crossings > 1:
                st = crossings / len(cl_hist) * D / u_in
        ...
        print(f"  St = {st:.4f}  (ref~0.20)")
        passed = err < 30.0
        print(f"\n{'PASS' if passed else 'FAIL'}: Cd err={err:.1f}% (target < 30%)")
        return passed
    return False
