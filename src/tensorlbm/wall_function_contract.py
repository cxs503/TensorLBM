...
def wall_function_capability_matrix() -> Mapping[
    WallFunctionCapability, WallFunctionCapabilityRecord
]:
    ...
    return MappingProxyType(
        {
            WallFunctionCapability.DISTANCE_FMM: WallFunctionCapabilityRecord(
                ...
                frozenset({"MASK_2D", "MASK_3D"}),
                frozenset({"mask_only"}),
                frozenset({"none"}),
                frozenset({"static_voxel_solid"}),
                frozenset({"torch"}),
            ...
            WallFunctionCapability.LOG_LAW_BODY_FORCE: WallFunctionCapabilityRecord(
                ...
                frozenset({"D3Q19"}),
                frozenset({"single_phase_incompressible"}),
                frozenset({"MRT_SMAGORINSKY"}),
                frozenset({"static_voxel_solid"}),
                frozenset({"torch"}),
                ValidationLevel.IMPLEMENTATION_ONLY,
            ...
            WallFunctionCapability.MOVING_BOUNCE_BACK: WallFunctionCapabilityRecord(
                ...
                frozenset({"single_phase_incompressible"}),
                frozenset({"BGK", "MRT"}),
                frozenset({"static_voxel_solid", "moving_voxel_solid"}),
                frozenset({"torch"}),
                ValidationLevel.IMPLEMENTATION_ONLY,
            ...
            WallFunctionCapability.COMMON_WALL_FUNCTION: WallFunctionCapabilityRecord(
                ...
                frozenset({"D3Q19", "D3Q27"}),
                frozenset({"single_phase_incompressible"}),
                frozenset({"BGK", "MRT", "CM", "KBC"}),
                frozenset({"static_voxel_solid"}),
                frozenset({"torch"}),
                ValidationLevel.IMPLEMENTATION_ONLY,
...
def assess_wall_function(request: WallFunctionRequest) -> WallFunctionAssessment:
    ...
    dimensions = (
        (request.lattice, record.lattices),
        (request.physics, record.physics),
        (request.collision, record.collisions),
        (request.geometry, record.geometries),
        (request.backend, record.backends),
    )
    if any(value not in allowed for value, allowed in dimensions):
        return WallFunctionAssessment(
            False,
            None,
...
__all__ = [
    "WITHHELD_UNVERIFIED_COMBINATION",
    "WITHHELD_VALIDATION_LEVEL",
    "ValidationLevel",
    "WallFunctionAssessment",
