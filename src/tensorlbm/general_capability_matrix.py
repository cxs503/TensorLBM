...
_COMPONENT_EVIDENCE: dict[str, dict[str, tuple[bool, EvidenceTier, str]]] = {
    "lattice": {
        "d3q19": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "D3Q19 MRT entrypoint is executable: tensorlbm.solver3d.collide_mrt3d.",
        ),
        "d3q27": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "D3Q27 MRT entrypoint is executable: tensorlbm.d3q27.collide_mrt27.",
        ),
    },
    "collision": {
        "mrt": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "advanced_collision_contract advertises MRT for D3Q19 and D3Q27.",
        ),
        "cm": (
            False,
            EvidenceTier.UNIMPLEMENTED,
            "CM is explicitly withheld: no standalone validated central-moment kernel.",
        ),
        "kbc": (
            False,
            EvidenceTier.UNIMPLEMENTED,
            "KBC is explicitly withheld: no entropy-solved KBC kernel.",
        ),
    },
    "multiphase": {
        "single_phase": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "Single-phase is the default; no multiphase coupling required.",
        ),
        "shan_chen": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "D3Q27 Shan-Chen multiphase collision is executable: tensorlbm.multiphase3d_d3q27.collide_sc_single_component_27 / collide_sc_two_component_27.",
        ),
        "free_surface": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "Free-surface LBM is implemented: tensorlbm.free_surface_lbm.",
        ),
        "phase_field": (
            True,
            EvidenceTier.COMPONENT_CONTRACT,
            "Phase-field LBM is implemented: tensorlbm.phasefield.free_energy.",
        ),
    },
}
...
def assess_capability(candidate: CapabilityRequest | Mapping[str, Any]) -> CapabilityAssessment:
    ...
    if reasons:
        return CapabilityAssessment(
            ...
            tuple(reasons),
            EvidenceTier.UNKNOWN_REQUEST,
            capability_hash(),
            _hash(asdict(request)),
            request,
        )
    ...
    for field in ("lattice", "collision"):
        ...
        if entry is None:
            unsupported = True
            reasons.append(
                CapabilityReason("UNKNOWN_VALUE", field, f"{value!r} is not defined by R1.")
            )
        elif not entry[0]:
            withheld = True
    ...
    for field in (
        ...
        if actual[field] != expected[field]:
            ...
            reasons.append(
                CapabilityReason(
                    "WITHHELD_UNVERIFIED_COMPOSITION",
                    field,
    ...
    if unsupported:
        ...
        tier = (
            EvidenceTier.UNIMPLEMENTED
            if any(reason.code == "WITHHELD_COLLISION_FAMILY" for reason in reasons)
            else EvidenceTier.NO_COMPOSITION_EVIDENCE
        )
    else:
        status, tier = CapabilityStatus.SUPPORTED, EvidenceTier.EXECUTABLE_CONTRACT
        reasons.append(
            CapabilityReason(
                "R1_VERIFIED_COMPOSITION",
                "candidate",
                "D3Q19/MRT single-phase static-wall, bounce-back, no-AMR Torch composition is admitted by R1.",
            )
        )

    return CapabilityAssessment(
        status, tuple(reasons), tier, capability_hash(), _hash(asdict(request)), request
    )
