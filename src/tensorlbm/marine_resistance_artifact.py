...
def build_marine_resistance_artifact(
    ...
    if provenance.get("schema") != "marine-run-provenance-v1" or provenance.get(
        "observation_sha256"
    ) != _digest(observation):
    ...
    if reference.get("schema") != "marine-reference-manifest-v1" or reference.get(
        "sha256"
    ) != _digest(unsigned_reference):
    ...
    if (
        ...
        or not isfinite(float(coefficient))
    ):
