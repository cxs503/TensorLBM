...
def _scalar(value: object, name: str, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    ...
    if value.dtype != dtype or value.device != device or not bool(torch.isfinite(value)):
        raise PopulationTransferPlanError(
            f"{name} must be finite and match population dtype/device"
        )
    return value
...
def validate_i_to_g_population_transfer(
    ...
    if not isinstance(event, IToGPopulationTransferEvent):
        return _withheld(
            None, "WITHHELD_INVALID_TRANSFER_EVENT", "IToGPopulationTransferEvent is required"
        )
    ...
    if any(not isinstance(partition, tuple) or not partition for partition in partitions):
        return _withheld(
            event_id, "WITHHELD_MISSING_PARTITION", "all event partitions must be non-empty tuples"
        )
    if any(any(not _valid_cell(cell) for cell in partition) for partition in partitions):
        return _withheld(
            event_id,
            "WITHHELD_INVALID_PARTITION_CELL",
            "partitions require integer (z, y, x) cells",
        )
    if len(set(event.source_partition)) != len(event.source_partition) or len(
        set(event.destination_partition)
    ) != len(event.destination_partition):
        return _withheld(
            event_id,
            "WITHHELD_DUPLICATE_PARTITION_CELL",
            "source and destination partitions must be unique",
        )
    if set(event.source_partition) & set(event.destination_partition):
        return _withheld(
            event_id,
            "WITHHELD_OVERLAPPING_PARTITIONS",
            "source and destination partitions must not overlap",
        )
    if set(event.converting_donors) != set(event.source_partition):
        return _withheld(
            event_id,
            "WITHHELD_SOURCE_CONVERSION_MISMATCH",
            "every and only converting I→G donor must be a source",
        )
    if (
        event.source_phase_before,
        event.source_phase_after,
        event.destination_phase_before,
        event.destination_phase_after,
    ) != (Phase.I, Phase.G, Phase.I, Phase.I):
        return _withheld(
            event_id,
            "WITHHELD_INVALID_OWNERSHIP_PARTITION",
            "R1 requires I→G sources and surviving I destinations",
        )
    if not isinstance(evidence, PopulationTransferEvidence):
        return _withheld(
            event_id,
            "WITHHELD_MISSING_POPULATION_EVIDENCE",
            "explicit pre/post donor and receiver f tensors are required",
        )
    populations = (
        evidence.donor_before,
        evidence.donor_after,
        evidence.receiver_before,
        evidence.receiver_after,
    )
    if any(not isinstance(value, torch.Tensor) for value in populations):
        return _withheld(
            event_id, "WITHHELD_INVALID_POPULATION_EVIDENCE", "population evidence must be tensors"
        )
    first = evidence.donor_before
    expected_donor, expected_receiver = (
        (len(event.source_partition), Q),
        (len(event.destination_partition), Q),
    )
    if (
        first.shape != expected_donor
        or evidence.donor_after.shape != expected_donor
        or evidence.receiver_before.shape != expected_receiver
        or evidence.receiver_after.shape != expected_receiver
    ):
        return _withheld(
            event_id,
            "WITHHELD_POPULATION_SHAPE_MISMATCH",
            "population tensors must be (partition-size, 19)",
        )
    if any(
        ...
        return _withheld(
            event_id,
            "WITHHELD_INVALID_POPULATION_VALUES",
            "populations must be finite and share dtype/device",
        )
    try:
        ...
        recorded_mass_residual = _scalar(
            ledger.residual, "independent_mass.residual", first.dtype, first.device
        )
    ...
    if not torch.equal(recorded_mass_residual, actual_mass_residual):
        return _withheld(
            event_id,
            "WITHHELD_INDEPENDENT_MASS_RESIDUAL_TAMPERED",
            "independent-mass residual must equal separate debit plus credit",
        )
    population_residual = (evidence.donor_after - evidence.donor_before).sum() + (
        evidence.receiver_after - evidence.receiver_before
    ).sum()
    population_zero = bool(population_residual == 0)
    mass_zero = bool(actual_mass_residual == 0)
    if not population_zero:
        return PopulationTransferValidation(
            "WITHHELD_POPULATION_RESIDUAL_NONZERO",
            "post-minus-pre f population sum is nonzero; direct copy/reset is not conservative evidence",
            event_id,
            population_residual,
            actual_mass_residual,
            False,
            mass_zero,
            False,
        )
    if not mass_zero:
        return PopulationTransferValidation(
            "WITHHELD_INDEPENDENT_MASS_RESIDUAL_NONZERO",
            "independent mass debit/credit does not close",
            event_id,
            population_residual,
            actual_mass_residual,
            True,
    ...
    return PopulationTransferValidation(
        ...
        "R1 verified explicit evidence but defines no f transfer operator; no copy, overwrite, or magic correction is authorized",
        event_id,
        population_residual,
        actual_mass_residual,
        True,
...
def plan_i_to_g_population_transfer(
    ...
    evidence: object,
    *,
    policy: Literal["unspecified"] | None = None,
    ...
    if policy not in (None, "unspecified"):
        validation = PopulationTransferValidation(
            "WITHHELD_UNSUPPORTED_TRANSFER_POLICY",
            "R1 has no validated executable f transfer policy",
            validation.event_id,
            validation.population_residual,
            validation.independent_mass_residual,
            validation.population_machine_zero,
            validation.independent_mass_machine_zero,
            False,
        )
