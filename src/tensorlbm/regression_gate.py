...
def evaluate_acoustic_campaign_gate(
    artifacts_root: str | Path, specification: dict[str, Any]
) -> dict[str, Any]:
    ...
    for name, raw_case in cases.items():
        ...
        if len(matching_status) != 1:
            ...
            row.update(
                {
                    section: {"pass": False}
                    for section in ("completion", "artifacts", "metrics", "physics")
                }
            )
            row["pass"] = False
            rows.append(row)
        ...
        metric_patterns = _required_regexes(
            spec.get("required_metrics"), "required_metrics", case_errors
        )
        matched_metrics = [
            pattern for pattern in metric_patterns if re.search(pattern, text, re.MULTILINE)
        ]
        row["metrics"] = {
            "pass": bool(metric_patterns) and len(matched_metrics) == len(metric_patterns),
        ...
        row["pass"] = all(
            row[section]["pass"] for section in ("completion", "artifacts", "metrics", "physics")
        )
        rows.append(row)
    ...
    return {
        ...
        "pass": bool(rows) and all(row["pass"] for row in rows) and summary_ok,
        "errors": errors,
    }
...
def _physics_result(
    ...
    if report_path is not None:
        ...
        if case_name is not None:
            ...
            match = next(
                (row for row in cases if isinstance(row, dict) and row.get("case") == case_name),
                None,
            )
            if match is None:
...
def _marine_binding_errors(artifact: dict[str, Any], case: str) -> list[str]:
    ...
    if (
        binding.get("reference_sha256") != reference_digest
        or reference.get("sha256") != reference_digest
    ):
        errors.append("binding does not match canonical reference")
    if (
        observation.get("schema") != "suboff-resistance-runtime-observation-v1"
        or observation.get("case") != case
    ):
    ...
    if (
        provenance.get("schema") != "marine-run-provenance-v1"
        or not isinstance(provenance.get("runner"), str)
        or not provenance["runner"]
        or provenance.get("observation_sha256") != observation_digest
    ):
        errors.append("provenance does not bind canonical observation")
    if (
        reference.get("schema") != "marine-reference-manifest-v1"
        or reference.get("case") != case
        or not _finite_number(reference.get("coefficient"))
        or float(reference["coefficient"]) <= 0.0
...
def _marine_failure_row(name: str, errors: list[str]) -> dict[str, Any]:
    return {
        ...
        "resistance": {"pass": False},
        "physics": {"pass": False},
        "pass": False,
    }


def evaluate_marine_resistance_gate(
    artifacts_root: str | Path, cases: dict[str, Any]
) -> dict[str, Any]:
    ...
    if not isinstance(cases, dict) or not cases:
        return {
            ...
            "cases": [],
            "pass": False,
            "errors": ["cases must be a non-empty object"],
        }
    ...
    for name, spec in cases.items():
        ...
        completion_ok = (
            completion.get("state") in _TERMINAL_SUCCESS
        ...
        missing_checks = [
            ...
            or not isinstance(checks.get(item), dict)
            or checks[item].get("pass") is not True
        ]
        ...
        numerics_ok = (
            ...
            and numerics.get("pass") is True
            and _is_finite_json(numerics)
        )
        conservation = (
            facts.get("conservation") if isinstance(facts.get("conservation"), dict) else {}
        )
        # Runtime v2 observations record the maximum drift measured across all
        # samples.  Retain v1 names only for legacy artifacts.
        mass_drift = conservation.get(
            "max_relative_mass_drift", conservation.get("mass_relative_drift")
        )
        momentum_drift = conservation.get(
            "max_relative_momentum_drift", conservation.get("momentum_relative_drift")
        )
        conservation_ok = (
            conservation.get("pass") is True
            and _finite_number(mass_drift)
            and _finite_number(momentum_drift)
        ...
        coefficient, reference, reported_error_pct = (
            resistance.get("coefficient"),
            resistance.get("reference_coefficient"),
            resistance.get("relative_error_pct"),
        )
        computed_error_pct = None
        coefficients_ok = (
            _finite_number(coefficient)
        ...
        if coefficients_ok:
            computed_error_pct = (
                abs(float(coefficient) - float(reference)) / float(reference) * 100.0
            )
        reported_error_ok = _finite_number(reported_error_pct) and float(reported_error_pct) >= 0.0
        error_matches_coefficients = (
            computed_error_pct is not None
            and reported_error_ok
            and math.isclose(
                float(reported_error_pct), computed_error_pct, rel_tol=1.0e-9, abs_tol=0.0
            )
        )
        ...
        resistance_ok = (
            resistance.get("pass") is True
            and coefficients_ok
            and reported_error_ok
            and error_matches_coefficients
            and computed_error_pct is not None
        ...
        row = {
            ...
            "conservation": {
                ...
                "momentum_relative_drift": momentum_drift,
            },
            "resistance": {
                "pass": resistance_ok,
    ...
    return {
        ...
        "artifacts_root": str(root),
        "cases": report_cases,
        "pass": bool(report_cases) and all(row["pass"] for row in report_cases),
    }
...
def evaluate_regression_gate(artifacts_root: str | Path, cases: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, raw_spec in cases.items():
        ...
        row["pass"] = all(
            bool(row[section]["pass"])
            for section in ("completion", "artifacts", "numerics", "physics")
        )
        report_cases.append(row)
    return {
        "schema_version": 1,
