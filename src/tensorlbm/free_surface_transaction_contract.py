...
def _valid_cell(cell: object) -> bool:
    return (
        ...
        and len(cell) == 3
        and all(
            isinstance(coordinate, int) and not isinstance(coordinate, bool) for coordinate in cell
        )
    )
...
def diagnose_korner_i_to_g_transaction(transaction: object) -> TransactionContractReport:
    ...
    if not event_id or not event_id.strip():
        return _withheld(
            "WITHHELD_MISSING_EVENT_ID", "transaction input requires a non-empty event id", event_id
        )
    ...
    if not transaction.conversions:
        return _withheld(
            "WITHHELD_MISSING_CONVERSION_CELLS",
            "transaction input requires conversion cells",
            event_id,
        )
    if any(
        not isinstance(item, CellConversion)
        or not _valid_cell(item.cell)
        or item.before is not CellState.I
        or item.after is not CellState.G
        for item in transaction.conversions
    ):
        return _withheld(
            "WITHHELD_INVALID_I_TO_G_CONVERSION",
            "every conversion must be a valid I→G cell",
            event_id,
        )
    converting_cells = {item.cell for item in transaction.conversions}
    if not transaction.donor_ownership:
        return _withheld(
            "WITHHELD_MISSING_DONOR_OWNERSHIP",
            "transaction input requires donor ownership",
            event_id,
        )
    donors = {
        item.cell for item in transaction.donor_ownership if isinstance(item, OwnershipEvidence)
    }
    if converting_cells != donors or any(
        not _valid_cell(item.cell)
        or item.state is not CellState.I
        or item.independent_mass_owner != "independent_mass"
        or item.population_owner != "f"
        for item in transaction.donor_ownership
    ):
        return _withheld(
            "WITHHELD_INVALID_DONOR_OWNERSHIP",
            "each converting I cell requires mass and f ownership",
            event_id,
        )
    if not transaction.receiver_ownership:
        return _withheld(
            "WITHHELD_MISSING_RECEIVER_OWNERSHIP",
            "transaction input requires receiver ownership",
            event_id,
        )
    receiver_cells = {
        item.cell for item in transaction.receiver_ownership if isinstance(item, OwnershipEvidence)
    }
    if any(
        not isinstance(item, OwnershipEvidence)
        or not _valid_cell(item.cell)
        or item.state is not CellState.I
        or item.independent_mass_owner != "independent_mass"
        or item.population_owner != "f"
        ...
        return _withheld(
            "WITHHELD_INVALID_RECEIVER_OWNERSHIP",
            "each receiver requires I-state mass and f ownership",
            event_id,
        )
    ...
    if not converting_cells.issubset(set(transfer.source_cells)):
        return _withheld(
            WITHHELD_NO_POPULATION_TRANSFER,
            "every converting donor requires actual f-transfer source evidence",
            event_id,
        )
    roundoff = transaction.roundoff
    if not isinstance(roundoff, RoundoffResidualEvidence):
        return _withheld(
            "WITHHELD_MISSING_ROUNDOFF_EVIDENCE",
            "transaction input requires roundoff evidence",
            event_id,
        )
    if (
        not isinstance(roundoff.residual, float)
        or not isfinite(roundoff.residual)
        or roundoff.residual != 0.0
    ):
        return _withheld(
    ...
    if roundoff.claimed_exact_closure is not True:
        return _withheld(
            "WITHHELD_EXACT_CLOSURE_NOT_CLAIMED",
            "exact diagnostic closure must be explicitly evidenced",
            event_id,
        )
