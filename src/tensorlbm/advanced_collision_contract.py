...
def collision_capability_matrix() -> dict[LatticeName, dict[CollisionFamily, CollisionCapability]]:
    ...
    return {
        "D3Q19": {
            "BGK": CollisionCapability(
                True,
                "tensorlbm.solver3d.collide_bgk3d",
                "AVAILABLE",
                "Single-relaxation-time BGK; conserved moments are exact.",
            ),
            "TRT": CollisionCapability(
                True,
                "tensorlbm.solver3d.collide_trt3d",
                "AVAILABLE",
                "Two-relaxation-time with magic-parameter Λ; symmetric/anti-symmetric split via OPPOSITE.",
            ),
            "RLBM": CollisionCapability(
                True,
                "tensorlbm.solver3d.collide_rlbm3d",
                "AVAILABLE",
                "Regularized BGK; non-equilibrium projected onto 2nd-order Hermite subspace.",
            ),
            "MRT": CollisionCapability(
                True,
                "tensorlbm.solver3d.collide_mrt3d",
                "AVAILABLE",
                "19x19 MRT transform; conserved rows are explicit.",
            ),
            "CM": CollisionCapability(
                True,
                "tensorlbm.cascaded_collision.collide_cascaded_d3q19",
                "AVAILABLE",
                "Full cascaded central-moment hierarchy (0th-4th order).",
            ),
            "CUMULANT": CollisionCapability(
                True,
                "tensorlbm.cumulant.collide_cumulant_d3q19",
                "AVAILABLE",
                "D3Q19 cumulant collision kernel.",
            ),
            "KBC": CollisionCapability(
                True,
                "tensorlbm.entropic_kbc.collide_kbc_d3q19",
                "AVAILABLE",
                "Entropic KBC with per-cell entropy-minimised gamma, KBC decomposition (kinetic/shear/higher-order), and positivity admissibility.",
            ),
        },
        "D3Q27": {
            "BGK": CollisionCapability(
                True,
                "tensorlbm.d3q27.collide_bgk27",
                "AVAILABLE",
                "Single-relaxation-time BGK; conserved moments are exact.",
            ),
            "TRT": CollisionCapability(
                True,
                "tensorlbm.d3q27.collide_trt27",
                "AVAILABLE",
                "Two-relaxation-time with magic-parameter Λ; symmetric/anti-symmetric split via D3Q27 OPPOSITE (includes corner directions).",
            ),
            "RLBM": CollisionCapability(
                True,
                "tensorlbm.d3q27.collide_rlbm27",
                "AVAILABLE",
                "Regularized BGK; 2nd-order Hermite projection with D3Q27 4th-order-isotropic weights.",
            ),
            "MRT": CollisionCapability(
                True,
                "tensorlbm.d3q27.collide_mrt27",
                "AVAILABLE",
                "27x27 full-rank Gram-Schmidt moment transform with explicit inverse.",
            ),
            "CM": CollisionCapability(
                True,
                "tensorlbm.cascaded_collision.collide_cascaded_d3q27",
                "AVAILABLE",
                "Full cascaded central-moment hierarchy (0th-6th order).",
            ),
            "CUMULANT": CollisionCapability(
                True,
                "tensorlbm.cumulant.collide_cumulant_d3q27",
                "AVAILABLE",
                "D3Q27 cumulant collision kernel.",
            ),
            "KBC": CollisionCapability(
                True,
                "tensorlbm.entropic_kbc.collide_kbc_d3q27",
                "AVAILABLE",
                "Entropic KBC with per-cell entropy-minimised gamma, KBC decomposition (kinetic/shear/higher-order), and positivity admissibility.",
            ),
        },
...
def _normalise_family(family: str) -> CollisionFamily:
    ...
    aliases = {
        ...
        "CASCADED": "CM",
        "CUMULANT": "CUMULANT",
        "CUMULANT_LBM": "CUMULANT",
        "KBC": "KBC",
        "ENTROPIC_KBC": "KBC",
    }
    if value not in aliases:
        raise ValueError(
            "family must be BGK/SRT, TRT, RLBM/regularized, MRT, CM/cascaded, or CUMULANT, or KBC/entropic_kbc"
        )
...
def _select_kernel(
    lattice_name: LatticeName, family_name: CollisionFamily
...
def collide_advanced_3d(
    lattice: str, family: str, f: torch.Tensor, *, tau: float | torch.Tensor, **rates: float
) -> torch.Tensor:
...
__all__ = [
    ...
    "WITHHELD_NO_D3Q19_CM_KERNEL",
    "WITHHELD_NO_D3Q19_KBC_KERNEL",
