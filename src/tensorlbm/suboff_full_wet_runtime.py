...
class SuboffFullWetRuntimeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.velocity, tuple)
            or len(self.velocity) != 3
            or any(
                isinstance(value, bool)
                or not isinstance(value, (int, float))
                or not isfinite(float(value))
...
def run_suboff_full_wet_runtime(
    ...
    *,
    ...
    velocities = [
        torch.full(shape, float(component), dtype=torch.float64)
        for component in resolved_config.velocity
    ]
    ...
    contract = build_resistance_force_contract(
        ...
        U=resolved_config.U,
        direction=resolved_config.velocity,
        method=method,
        sample_phase=sample_phase,
    ...
    case_record = {
        "application": resolved_case.application,
        "configuration": resolved_case.configuration,
        "medium": resolved_case.medium,
        "schema": resolved_case.schema,
        "reference_sha256": resolved_case.reference.sha256,
    }
    config_record = asdict(resolved_config)
    return {
        "artifact_kind": "software_runtime",
        "schema": "suboff-full-wet-runtime-r1",
        "case": case_record,
        "case_hash": _hash(case_record),
        "geometry": {
            "body_id": asset.body_id,
            "source_hash": asset.source_hash,
            "shape_zyx": tuple(shape),
        },
        "runtime_config_hash": _hash(config_record),
        "method": method,
        "sample_phase": sample_phase,
        "links": {"count": links.count, "ownership": ownership},
        "force_series": series,
        "force": mean_force,
        "Ct": contract.Ct,
        "status": contract.status,
        "force_observation": _observation_dict(observation),
