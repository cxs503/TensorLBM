#!/usr/bin/env python3
"""FSI rotating turbine fix — SDAA:11.

Fix:
  1. build_turbine_mask: use torch.zeros_like(r, dtype=torch.bool)
     (was torch.zeros_like(r) which creates float tensor —
      boolean |= operations on float tensors don't work correctly → NaN)
  2. Reduce TSR to 2 (tip speed < 0.1 for incompressible stability)

3-blade turbine, Re=1000, D=60
10000 steps
Target: Cp > 0, stable
"""
...
def build_turbine_mask(nx, ny, nz, cx, cy, R_blade, n_blades, blade_width,
                       angle_deg, device):
    """3-blade turbine mask (rotated by angle_deg).

    FIX: use torch.zeros_like(r, dtype=torch.bool) instead of
    torch.zeros_like(r) which creates a float tensor.
    """
    ...
    # FIX: use dtype=torch.bool (was torch.zeros_like(r) → float)
    blade = torch.zeros_like(r, dtype=torch.bool)
    for i in range(n_blades):
        ...
        blade |= (xb >= -R_blade) & (xb <= R_blade) & \
                 (abs(yb) <= blade_width / 2) & (r <= R_blade)

    # Hub
    hub_r = max(2, R_blade // 6)
    blade |= r <= hub_r

    solid = blade.unsqueeze(0).expand(nz, ny, nx).clone()
    return solid
...
def run_rotating(device_id, output_path=None):
    ...
    tau = 3.0 * nu + 0.5
    n_steps = 10000
    tag = f"[Turbine-fix SDAA:{device_id}]"

    # FIX: Reduce TSR to 2 (tip speed = TSR * u_in = 0.1 < 0.1 threshold)
    tsr = 2.0
    omega = tsr * u_in / R_blade
    tip_speed = omega * R_blade  # = tsr * u_in = 0.1

    print(f"{tag} nx={nx} ny={ny} nz={nz} D={D} R={R_blade} Re={Re} "
          f"u_in={u_in} nu={nu:.6e} tau={tau:.6f}", flush=True)
    print(f"{tag} TSR={tsr} omega={omega:.6f} tip_speed={tip_speed:.4f} "
          f"(fixed-mask quasi-steady)", flush=True)

    dpS = 0.5 * u_in ** 2 * D * nz
    A_swept = math.pi * R_blade ** 2
    p_ref = 0.5 * u_in ** 3 * A_swept
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 2000 == 0:
            ...
            print(f"{tag} step={step}/{n_steps} Cd={at:.4f} Cl={cl_avg:.4f} "
                  f"torque={tq_avg:.2f} Cp={cp_avg:.4f} "
    ...
    result_dict = {
        "case": "rotating_turbine_Re1000_fixed",
        "device": f"sdaa:{device_id}",
        "grid": f"{nx}x{ny}x{nz}",
        "D": D, "R_blade": R_blade, "n_blades": n_blades,
        "Re": Re, "u_in": u_in, "nu": nu, "tau": tau,
        "TSR": tsr, "omega": omega, "tip_speed": tip_speed,
...
if __name__ == "__main__":
    dev = int(sys.argv[1]) if len(sys.argv) > 1 else 11
