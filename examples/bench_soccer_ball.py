...
Sphere drag at Re=1e5. Cd≈0.2 (super-critical).
...
def build_sphere(nx, ny, nz, device="cpu"):
    R = ny // 5
    cx = nx // 3
    cy = ny // 2
    cz = nz // 2
    zz, yy, xx = torch.meshgrid(torch.arange(nz), torch.arange(ny), torch.arange(nx), indexing="ij")
    solid = torch.sqrt((xx - cx) ** 2 + (yy - cy) ** 2 + (zz - cz) ** 2) < R
    return solid.to(device)


def run_soccer_ball(device="sdaa:8", n_steps=1500, warmup=400):
    dev = torch.device(device)
    nx, ny, nz = 192, 128, 128
    u_in = 0.06
    re = 1e5
    nu_lat = u_in * ny * 0.2 / re
    tau = 3.0 * nu_lat + 0.5
    solid = build_sphere(nx, ny, nz, device="cpu").to(dev)
    fluid = ~solid
    R = ny // 5
    S = math.pi * R**2
    ...
    print(f"Soccer Ball (足球阻力): Re={re:.0e} grid={nx}x{ny}x{nz} Cd_ref=0.2", flush=True)
    fric = []
    pres = []
    import time
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            print(f"  step {step}: Cd={cf + cp:.4f} (ref=0.2)", flush=True)
    dt = time.time() - t0
    cf = sum(fric) / max(len(fric), 1) / dyn_p_S
    cp = sum(pres) / max(len(pres), 1) / dyn_p_S
    print_cp_report(f, solid, u_in, geometry_type="sphere", cx=cx, cy=cy)
    print(f"Final: Cd={cf + cp:.4f} ref=0.2 {dt:.0f}s", flush=True)
...
if __name__ == "__main__":
    ...
    a.add_argument("--device", default="sdaa:8")
    args = a.parse_args()
    run_soccer_ball(device=args.device)
