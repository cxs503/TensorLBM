...
def run_hull_fs_d3q27(
    ...
    solid, stats = build_hull_mask(
        hull_type, nx, ny, nz, cx=int(nx * 0.3), cy=ny // 2, cz_keel=fill_height - 10, device="cpu"
    )
    solid = solid.to(dev)
    fluid = ~solid
    ...
    f = equilibrium27(rho_init, ux_init, torch.zeros_like(ux_init), torch.zeros_like(ux_init))
    g = _init_g_equilibrium(
        phi, ux_init, torch.zeros_like(ux_init), torch.zeros_like(ux_init), c, w
    )
    ...
    for step in range(1, n_steps + 1):
        ...
        if use_wall_function:
            ...
            f, df = _wall_function_3d(
                f, solid, near_water, fluid, c, cx3d, cy3d, cz3d, w, cs2, nu_lat
            )
        ...
        if step % 200 == 0 or step == n_steps:
            ...
            print(
                f"  step {step:5d}: Cf={cf:.5f} Cw={cw:.5f} Ct={ct:.5f} "
