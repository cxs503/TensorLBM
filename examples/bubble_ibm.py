...
def run_ibm_bubble(nx=64, ny=96, nz=16, n_steps=3000, device="sdaa:0"):
    ...
    for step in range(1, n_steps + 1):
        ...
        fx, fy, fz = ibm_direct_forcing_3d(
            ux_now, uy_now, uz_now, mx, my, mz, u_target_x, u_target_y, u_target_z, kernel="hat"
        )
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            print(
                f"step {step:4d}: cy={cy:.1f} vy={vy_bubble:.4f} "
                f"F_buoy={F_buoyancy:.2f} F_drag={F_drag:.2f} "
                f"p_top={p_top:.4f} p_bot={p_bot:.4f} "
                f"rho_l=[{rho_min:.3f},{rho_max:.3f}]",
                flush=True,
            )

    dt = time.time() - t0
    print(
        f"\nFinal: cy={cy:.1f} vy={vy_bubble:.4f} {dt:.1f}s ({dt / n_steps * 1000:.0f}ms/step)",
        flush=True,
    )
