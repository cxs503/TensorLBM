...
Staggered tube bundle. Re=1000-10000.
Pressure drop benchmark for heat exchangers.
"""
...
def build_tube_bundle(nx, ny, nz, device="cpu"):
    R = ny // 12
    pitch = ny // 4
    ...
    for i in range(nx // pitch + 2):
        for j in range(ny // pitch + 2):
            cx = i * pitch + pitch // 2 + (pitch // 2 if j % 2 else 0)
            cy = j * pitch + pitch // 2
            solid |= ((xx - cx) ** 2 + (yy - cy) ** 2) < R**2
    return solid.to(device)


def run_tube_bundle(device="sdaa:5", n_steps=1500, warmup=400):
    dev = torch.device(device)
    nx, ny, nz = 256, 128, 8
    u_in = 0.06
    re = 5e3
    nu_lat = u_in * ny * 0.1 / re
    tau = 3.0 * nu_lat + 0.5
    solid = build_tube_bundle(nx, ny, nz, device="cpu").to(dev)
    fluid = ~solid
    R = ny // 12
    S = math.pi * R**2
    ...
    w27 = torch.tensor(
        [8 / 27] + [2 / 27] * 6 + [1 / 54] * 12 + [1 / 216] * 8, dtype=torch.float32, device=dev
    ).view(27, 1, 1, 1)
    cs2 = 1.0 / 3.0
    ...
    print(f"Tube Bundle (换热器压降): Re={re:.0e} grid={nx}x{ny}x{nz} Cd_ref=1.5", flush=True)
    fric = []
    pres = []
    import time
    ...
    for step in range(1, n_steps + 1):
        ...
        y_plus = 0.5 * u_tau / nu_lat
        turb = (y_plus > 11.6) & near
        if bool(turb.any()):
            ut = u_tau[turb].clone()
            um = u_mag[turb]
            for _ in range(8):
                lyp = torch.log(0.5 * ut / nu_lat)
        ...
        tau_w = u_tau * u_tau
        inv_umag = 1.0 / u_mag
        coef = -(tau_w / 0.5) * near.to(f.dtype)
        fx = coef * (ux * inv_umag)
        fy = coef * (uy * inv_umag)
        fz = coef * (uz * inv_umag)
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            print(f"  step {step}: Cd={cf + cp:.4f} (ref=1.5)", flush=True)
    dt = time.time() - t0
    cf = sum(fric) / max(len(fric), 1) / dyn_p_S
    cp = sum(pres) / max(len(pres), 1) / dyn_p_S
    print_cp_report(f, solid, u_in, geometry_type="generic")
    print(f"Final: Cd={cf + cp:.4f} ref=1.5 {dt:.0f}s", flush=True)
...
if __name__ == "__main__":
    ...
    a.add_argument("--device", default="sdaa:5")
    args = a.parse_args()
    run_tube_bundle(device=args.device)
