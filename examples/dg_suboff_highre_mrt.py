...
def run(
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 100 == 0 or step == n_steps:
            ...
            ct = (
                (sum(drag_samples) / max(len(drag_samples), 1)) / dyn_p_S
                if drag_samples
                else float("nan")
            )
            print(
                f"  step {step:4d}: drag_lu={drag:8.3f}  Ct={ct:.4f}  max|u|={ms:.4f}  {'UNSTABLE' if (not math.isfinite(ms) or ms > 1.0) else ''}"
            )
