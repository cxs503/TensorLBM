...
def suboff_checkpoint_metadata(*, nx, ny, nz, hull_length, re, u_in, y_val, world_size, rank):
    if nx % world_size:
...
def save_suboff_checkpoint(
    ...
    *,
    ...
    payload = {
        ...
        "mass_cadence": int(mass_cadence),
        "friction_sum": float(friction_sum),
        "pressure_sum": float(pressure_sum),
...
def run_multicard(
    ...
    mass_cadence = 100
    if resume_path:
        f, state = load_suboff_checkpoint(
            resume_path,
    ...
    for step in range(start_step + 1, n_steps + 1):
        ...
        if checkpoint_path and checkpoint_every and step % checkpoint_every == 0:
            save_suboff_checkpoint(
                checkpoint_path,
                f=f,
                step=step,
                metadata=checkpoint_metadata,
                target_mass=target_mass,
                mass_cadence=mass_cadence,
                friction_sum=fric_sum,
                pressure_sum=pres_sum,
    ...
    cp = pres_sum / max(drag_samples, 1) / dyn_p_S
    total = time.time() - t0
    completed = max(n_steps - start_step, 1)
    avg = t_step_total / completed
    mlups = total_cells / avg / 1e6
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--checkpoint", default=None, help="directory for rank-owned restart checkpoints"
    )
    p.add_argument("--checkpoint-every", type=int, default=None)
    p.add_argument("--resume", default=None, help="checkpoint directory to resume")
    p.add_argument(
        "--report-far-field-plan",
        action="store_true",
    ...
    if a.report_far_field_plan:
        print(
            suboff_far_field_plan(
                nx=a.nx, ny=a.ny, nz=a.nz, hull_length=a.hull, u_in=a.u_in, transient_steps=a.steps
            )
        )
    else:
        run_multicard(
