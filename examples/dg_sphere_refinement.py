...
def run(radius, use_band, tau_lbm=0.8, n_steps=1000, device="cuda"):
    ...
    for step in range(1, n_steps + 1):
        ...
        if use_band:
            ...
            f_lbm = collide_bgk3d(f_lbm, tau_lbm)
            f_lbm = stream3d(f_lbm)
        f_pre = f_lbm.clone()
        for d in range(1, 19):
            if bool(fluid_bc[d].any()):
                f_lbm = bouzidi_bounce_back_3d(f_lbm, f_pre, fluid_bc[d], q_field[d], d)
        f_lbm = apply_zou_he_channel_boundaries_3d(
            f_lbm, u_in=u_in, wall_mask=wall_mask, obstacle_mask=torch.zeros_like(solid)
        )
        if step % 200 == 0:
...
if __name__ == "__main__":
    ...
    for r in (8.0, 12.0):
        for use_band, label in ((False, "BFL-only"), (True, "BFL+DGband")):
            ...
            ref = cd_ref(re)
            print(
                f"{r:>7.0f} {label:>10} {re:>6.1f} {cd:>8.3f} {ref:>8.3f} {abs(cd - ref) / ref * 100:>7.1f}"
            )
