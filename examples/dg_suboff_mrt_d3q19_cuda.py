...
def run(
    ...
    print(
        f"{op_name}: Re={re:.0e} tau={tau:.5f} grid={nx}x{ny}x{nz} cells={nx * ny * nz:,} D={D:.1f}"
    )
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="SUBOFF wall-function solver CUDA (D3Q19 MRT / D3Q27 Cumulant)"
    )
    ...
    p.add_argument("--output_json", type=str, default=None, help="Save results to JSON file")
    a = p.parse_args()
    run(
        nx=a.nx,
        ny=a.ny,
