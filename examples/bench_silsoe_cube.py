...
6m cube building. Re=2e4.
Wind pressure distribution benchmark.
"""
...
def build_cube(nx,ny,nz,device='cpu'):
    S=ny//3; cx=nx//4; cy=ny//2; cz=0
    zz,yy,xx=torch.meshgrid(torch.arange(nz),torch.arange(ny),torch.arange(nx),indexing='ij')
    solid=(xx>=cx)&(xx<cx+S)&(yy>=cy-S/2)&(yy<cy+S/2)&(zz>=cz)&(zz<cz+S)
    return solid.to(device)

def run_silsoe_cube(device='sdaa:9', n_steps=1500, warmup=400):
    dev=torch.device(device)
    nx,ny,nz=192,96,96; u_in=0.06; re=2e4
    nu_lat=u_in*ny//3/re; tau=3.0*nu_lat+0.5
    solid=build_cube(nx,ny,nz,device='cpu').to(dev)
    fluid=~solid
    S_side=ny//3; S=S_side**2
    ...
    print(f'Silsoe Cube (建筑风压): Re={re:.0e} grid={nx}x{ny}x{nz} Cd_ref=0.6',flush=True)
    ...
    for step in range(1,n_steps+1):
        ...
        if step%300==0 or step==n_steps:
            ...
            print(f'  step {step}: Cd={cf+cp:.4f} (ref=0.6)',flush=True)
    dt=time.time()-t0
    cf=sum(fric)/max(len(fric),1)/dyn_p_S;cp=sum(pres)/max(len(pres),1)/dyn_p_S
    print_cp_report(f, solid, u_in, geometry_type='building', cx=cx, cy=cy)
    print(f'Final: Cd={cf+cp:.4f} ref=0.6 {dt:.0f}s',flush=True)

if __name__=='__main__':
    import argparse
    a=argparse.ArgumentParser()
    a.add_argument('--device',default='sdaa:9')
    args=a.parse_args()
    run_silsoe_cube(device=args.device)
