...
def collide_mrt_taueff_floor(f, tau_lam, nu_floor):
    ...
    s_fixed = torch.tensor(
        [0, 1.19, 1.4, 0, 1.2, 0, 1.2, 0, 1.2, 0, 0, 0, 0, 0, 1.19, 1.19, 1, 1, 1],
        dtype=f.dtype,
        device=device,
    )
...
def run(
    ...
    print(
        f"Re={re:.0e} tau_lam={tau:.5f} nu_floor={nu_floor} -> tau_eff>= {3 * (nu_lat + nu_floor) + 0.5:.3f}  S={S:.0f}"
    )
    samples = []
    for step in range(1, n_steps + 1):
        ...
        if step % 200 == 0 or step == n_steps:
            ...
            print(
                f"  step {step:4d}: drag={drag:8.3f} Ct={ct:.4f} max|u|={ms:.4f} {'UNSTABLE' if (not math.isfinite(ms) or ms > 1.0) else ''}"
            )
