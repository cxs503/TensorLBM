...
def run(
    ...
    fill = torch.where(
        zz < fill_height,
        torch.ones_like(zz, dtype=torch.float32),
    ...
    ux0 = torch.where(
        zz < fill_height,
        torch.full((nz, ny, nx), u_in, device=device),
        torch.zeros((nz, ny, nx), device=device),
    )
    ...
    for step in range(1, n_steps + 1):
        ...
        f = free_surface_step(
            f, fill, flags, solid_mask, tau=tau, gz=-gz, C_s=0.1, rho_liquid=1.0, rho_gas=0.0
        )
        ...
        if step > warmup:
            ...
            M_in = (
                (
                    rho_now[z0 : z1 + 1, y0 : y1 + 1, x0_cv]
                    * ux[z0 : z1 + 1, y0 : y1 + 1, x0_cv] ** 2
                )
                .sum()
                .item()
            )
            M_out = (
                (
                    rho_now[z0 : z1 + 1, y0 : y1 + 1, x1_cv]
                    * ux[z0 : z1 + 1, y0 : y1 + 1, x1_cv] ** 2
                )
                .sum()
                .item()
            )
            My0 = (
                (
                    rho_now[z0 : z1 + 1, y0, x0_cv : x1_cv + 1]
                    * ux[z0 : z1 + 1, y0, x0_cv : x1_cv + 1]
                    * uy[z0 : z1 + 1, y0, x0_cv : x1_cv + 1]
                )
                .sum()
                .item()
            )
            My1 = (
                (
                    rho_now[z0 : z1 + 1, y1, x0_cv : x1_cv + 1]
                    * ux[z0 : z1 + 1, y1, x0_cv : x1_cv + 1]
                    * uy[z0 : z1 + 1, y1, x0_cv : x1_cv + 1]
                )
                .sum()
                .item()
            )
            Mz0 = (
                (
                    rho_now[z0, y0 : y1 + 1, x0_cv : x1_cv + 1]
                    * ux[z0, y0 : y1 + 1, x0_cv : x1_cv + 1]
                    * uz[z0, y0 : y1 + 1, x0_cv : x1_cv + 1]
                )
                .sum()
                .item()
            )
            Mz1 = (
                (
                    rho_now[z1, y0 : y1 + 1, x0_cv : x1_cv + 1]
                    * ux[z1, y0 : y1 + 1, x0_cv : x1_cv + 1]
                    * uz[z1, y0 : y1 + 1, x0_cv : x1_cv + 1]
                )
                .sum()
                .item()
            )
        ...
        if step % 1000 == 0 or step == n_steps:
            ...
            print(
                f"  step={step:5d}  Ct_CV={ct:.5f}  (ref {cf_ref:.5f})  max|u|={ms:.4f}  "
