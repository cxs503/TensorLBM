...
def run_improved_propeller(
    nx=192,
    ny=96,
    nz=96,
    n_steps=5000,
    device="sdaa:0",
    mask_interval_deg=2.0,
    use_bfl=True,
    ...
    masks = [
        build_propeller_mask(
            nx, ny, nz, cx, cy, cz, angle_deg=i * mask_interval_deg, config=cfg, device="cpu"
        ).to(dev)
    ...
    for step in range(1, n_steps + 1):
        ...
        cu_wall = c[:, 1].view(27, 1, 1, 1) * u_wall_y.unsqueeze(0) + c[:, 2].view(
            27, 1, 1, 1
        ) * u_wall_z.unsqueeze(0)
        ...
        if bool(new_fluid.any()):
            ...
            prev_boundary = (
                prev_boundary & ~prev & ~solid
            )  # was fluid, adjacent to prev solid, still fluid
            ...
            if prev_boundary.any():
                ...
                has_nbr = cnt > 0
                ux_fill = torch.where(
                    has_nbr, ux_sum / cnt.clamp(min=1), torch.full_like(rho0, u_in)
                )
            ...
            feq_fill = equilibrium27(
                rho_fill.clamp(min=1e-6, max=3.0),
        ...
        if step % 500 == 0 or step == n_steps:
            ...
            print(
                f"step {step:5d}: KT={kt:.4f} (exp=0.37) n={len(samples)} "
                f"angle={math.degrees(angle) % 360:.1f}° {time.time() - t0:.0f}s",
                flush=True,
            )
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    run_improved_propeller(
        args.nx,
        args.ny,
        args.nz,
        args.steps,
        args.device,
        mask_interval_deg=args.mask_deg,
        use_bfl=not args.no_bfl,
    )
