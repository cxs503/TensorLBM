...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--nx", type=int, default=128, help="Grid size (square: nx × nx)")
    parser.add_argument(
        "--u-lid", dest="u_lid", type=float, default=0.1, help="Lid velocity in lattice units"
    )
    parser.add_argument(
        "--re", type=float, default=100.0, help="Reynolds number Re = u_lid * nx / nu"
    )
    parser.add_argument(
        "--n-steps", dest="n_steps", type=int, default=10000, help="Number of simulation steps"
    )
    parser.add_argument(
        "--output-interval", type=int, default=2000, help="Snapshot and diagnostic cadence"
    )
