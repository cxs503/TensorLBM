...
def run_cylinder(re=3900, nx=400, ny=160, D=40, u_in=0.1, n_steps=40000, device="cuda:0"):
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 4000 == 0 or step == n_steps:
            ...
            el = time.time() - t0
            print(
                f"  step {step:5d}: Cd={cd_avg:.4f} (ref {REF_CD}, "
    ...
    if len(t_shed) > 2:
        ...
        st = D / (T * u_in)
    print(f"\n  FINAL: Cd={cd_avg:.4f} (ref {REF_CD}, err {err:.1f}%)  St={st:.4f} (ref {REF_ST})")
    return {
        "re": re,
        "cd": cd_avg,
        "cd_ref": REF_CD,
...
if __name__ == "__main__":
    ...
    print("=" * 64)
    print(
        f"  Re={r['re']}: Cd={r['cd']:.4f} ref={r['cd_ref']:.4f} "
