...
def run_pf_csf_bubble(
    ...
    for step in range(1, n_steps + 1):
        ...
        # === 4. Density from phase field + gas mass conservation ===
        # ρ = ρ_gas + (ρ_liq - ρ_gas)·(φ+1)/2, where ρ_gas = p_gas/cs²
        # Pressure difference drives flow NATURALLY through LBM equilibrium (no CSF force)
        rho_gas = p_gas / cs2  # gas density from mass conservation
        ...
        feq_new = equilibrium3d(
            rho_field.clamp(min=1e-6, max=3.0),
        ...
        feq_old = equilibrium3d(
            rho_post.clamp(min=1e-6, max=3.0),
            ux_post.clamp(-0.5, 0.5),
            uy_post.clamp(-0.5, 0.5),
            uz_post.clamp(-0.5, 0.5),
            device=dev,
        ...
        if step % 100 == 0 or step == n_steps:
            ...
            status = (
                "EXPAND"
                if R_ratio > R_eq / R0 + 0.01
                else ("CONTRACT" if R_ratio < R_eq / R0 - 0.01 else "STABLE")
            )
            print(
                f"step {step:4d}: R={R_now:.3f} ratio={R_ratio:.4f} R_eq={R_eq / R0:.4f} "
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--p-ratio", type=float, default=1.5, help="p_gas0/p_liq (>1 expand, <1 contract)"
    )
    ...
    print(
        f"{'PF+CSF':16s} {R0:8.3f} {R_eq:8.3f} {Rs_lbm.min():8.3f} {Rs_lbm.max():8.3f} {Rs_lbm[-1]:8.3f}"
    )
    print()
