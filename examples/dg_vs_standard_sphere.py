...
def run(radius, use_dg, tau_lbm=0.78, u_in=0.1, cs=0.1, n_steps=2000, warmup=800, device="cuda"):
    ...
    for step in range(1, n_steps + 1):
        ...
        if use_dg:
            ...
            f = collide_smagorinsky_mrt3d(f, tau, cs)
            f = stream3d(f)
...
if __name__ == "__main__":
    ...
    print(
        f"{'radius':>7} {'Re':>5} {'std Cd':>8} {'err%':>6}  {'DG Cd':>8} {'err%':>6}  {'DG adv':>7}"
    )
    for r in (8.0, 12.0, 16.0):
        ...
        re = 0.1 * 2 * r / nu
        ref = cd_ref(re)
        e_std = abs(cd_std - ref) / ref * 100
        e_dg = abs(cd_dg - ref) / ref * 100
        adv = (e_std - e_dg) / e_std * 100 if e_std > 1e-9 else float("nan")
        print(
            f"{r:>7.0f} {re:>5.0f} {cd_std:>8.3f} {e_std:>6.1f}  {cd_dg:>8.3f} {e_dg:>6.1f}  {adv:>+7.1f}%"
        )
