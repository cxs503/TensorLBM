...
def run_bfl_fast(piston_speed=0.05, n_steps=4000, device="sdaa:9"):
    ...
    f = equilibrium3d(
        rho, torch.zeros_like(rho), torch.zeros_like(rho), torch.zeros_like(rho), device=dev
    )
    ...
    for step in range(1, n_steps + 1):
        ...
        if bool(near.any()):
            ...
            if q_val < 0.5:
                ...
                f_bfl = (
                    f_opp_near / (2.0 * q_s)
                    + (2.0 * q_s - 1.0) / (2.0 * q_s) * f_pre_opp_near
                    - corr
                )
        ...
        if step % 400 == 0 or step == n_steps:
            ...
            if not torch.isnan(rg).any():
                ...
                pe = rho0 * cs2 * cr
                print(
                    f"step {step}: q={q:.3f} V={cr:.2f} rho={ra:.4f} p={pa:.4f} "
                    f"p_exp={pe:.4f} ratio={pa / pe:.4f}",
                    flush=True,
                )
            else:
                print(f"step {step}: NaN! q={q:.3f}", flush=True)
                break
