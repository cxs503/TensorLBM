...
def gas_bfl_interface(f_g, f_g_pre, fill, ux_l, uy_l, uz_l, opp, c, w, cs2, gas_mask):
    ...
    for d in range(1, 19):
        ...
        f_opp = f_g[o][near_iface]
        f_pre_d = f_g_pre[d][near_iface]
        f_pre_opp = f_g_pre[o][near_iface]
        lin = q < 0.5
...
def run_sloshing(nx=96, ny=64, nz=8, n_steps=3000, device="sdaa:0", use_gas=True):
    ...
    print(
        f"Analytical omega={omega_analytical:.6f} period={2 * math.pi / omega_analytical:.0f} steps",
        flush=True,
    )
    print(flush=True)
    ...
    for step in range(1, n_steps + 1):
        ...
        if use_gas:
            ...
            f_g = stream3d(f_g)
            f_g = gas_bfl_interface(
                f_g, f_g_pre, fill, ux_l, uy_l, uz_l, opp, c, w, cs2, gas_domain
            )
            ...
            if step % 10 == 0:
                ...
                if cm > 1e-6 and not math.isnan(cm):
                    ...
                    if scale < 100:
                        f_g[:, gas_domain] *= scale
                f_g = f_g.clamp(min=0.0, max=3.0)
            # Gas → Liquid
            p_gas_field = f_g.sum(0) * cs2
            gas_full = (fill < 0.5) & ~solid
            p_nbr_sum = torch.zeros_like(p_gas_field)
            gas_nbr_cnt = torch.zeros_like(p_gas_field)
            ...
            p_at_iface = torch.where(
                gas_nbr_cnt > 0,
                p_nbr_sum / gas_nbr_cnt.clamp(min=1),
                torch.ones_like(p_gas_field) * cs2,
            )
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            print(
                f"step {step:4d}: L={left_h:.0f} R={right_h:.0f} diff={diff:.1f} "
                f"p_gas={p_gas:.4f} gas_mass={gas_mass:.0f}",
                flush=True,
            )
