...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode", choices=["2d", "3d"], default="2d", help="2D cylinder or 3D sphere"
    )
    parser.add_argument(
        "--model",
        choices=["cg", "sc"],
    ...
    parser.add_argument("--nz", type=int, default=80, help="z-dimension (3D mode only)")
    parser.add_argument("--radius", type=float, default=12.0, help="Sphere/cylinder radius (cells)")
    parser.add_argument(
        "--water-level", type=int, default=80, help="Initial water surface (y index in 2D, z in 3D)"
    )
    parser.add_argument(
        "--clearance",
        type=int,
