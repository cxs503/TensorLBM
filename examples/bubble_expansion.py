    ...
    for step in range(1, n_steps + 1):
        ...
        if bubble_surface.any():
            ...
            for dz, dy, dx in [(0, 0, 1), (0, 0, -1), (0, 1, 0), (0, -1, 0), (1, 0, 0), (-1, 0, 0)]:
                nbr_liquid = (
                    torch.roll(bubble_surface, shifts=(dz, dy, dx), dims=(0, 1, 2)) & liquid_now
                )
                if nbr_liquid.any():
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            if not torch.isnan(rg).any():
                ...
                print(
                    f"step {step}: R={R_current:.2f} V_ratio={1 / cr:.2f} "
                    ...
                    f"n_gas={int(bubble_now.sum())}",
                    flush=True,
                )
            else:
                print(f"step {step}: NaN!", flush=True)
                break
