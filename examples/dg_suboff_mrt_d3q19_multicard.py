...
def run_multicard(
    ...
    is_main = rank == 0
    with open(f"/tmp/mc_{rank}.log", "w") as _f:
        _f.write("setup done\n")
    ...
    del full_solid
    with open(f"/tmp/mc_{rank}.log", "a") as _f:
        _f.write("mask done\n")
    ...
    def wall_fn_19(f, nu, y_val=0.5):
        ...
        with open(f"/tmp/mc_{rank}.log", "a") as _f:
            _f.write("  wall_fn: before turb.any\n")
    ...
    for step in range(1, n_steps + 1):
        ...
        if step <= 3:
            with open(f"/tmp/mc_{rank}.log", "a") as _f:
                _f.write(f"step {step} before halo\n")
        halo_exchange(f, rank, world_size)
        f = stream19_roll(f)
        if step <= 3:
            with open(f"/tmp/mc_{rank}.log", "a") as _f:
                _f.write(f"step {step} after halo\n")
        f, df_local, dp_local = wall_fn_19(f, nu_lat, y_val=y_val)
        if step <= 3:
            with open(f"/tmp/mc_{rank}.log", "a") as _f:
                _f.write(f"step {step} after wall_fn\n")
        f = far_field_19(f, u_in=u_in)
        if step <= 3:
            with open(f"/tmp/mc_{rank}.log", "a") as _f:
                _f.write(f"step {step} after far_field\n")
        if step % 100 == 0:
        ...
        if step > warmup:
            ...
            if step <= 3:
                with open(f"/tmp/mc_{rank}.log", "a") as _f:
                    _f.write(f"step {step} before drag all_reduce\n")
            dist.all_reduce(drag_tensor, op=dist.ReduceOp.SUM)
            if step <= 3:
                with open(f"/tmp/mc_{rank}.log", "a") as _f:
                    _f.write(f"step {step} after drag all_reduce\n")
