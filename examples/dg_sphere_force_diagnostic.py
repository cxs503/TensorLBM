...
def cv_momentum_drag(rho, ux, uy, uz, cx, cy, cz, r):
    ...
    M += (
        (
            -rho[z0 : z1 + 1, y0 : y1 + 1, x0] * ux[z0 : z1 + 1, y0 : y1 + 1, x0] ** 2
            - p[z0 : z1 + 1, y0 : y1 + 1, x0]
        )
        .sum()
        .item()
    )
    M += (
        (
            rho[z0 : z1 + 1, y0 : y1 + 1, x1] * ux[z0 : z1 + 1, y0 : y1 + 1, x1] ** 2
            + p[z0 : z1 + 1, y0 : y1 + 1, x1]
        )
        .sum()
        .item()
    )
...
def run(
    ...
    f_eq_solid = equilibrium3d(
        rho0, torch.zeros_like(rho0), torch.zeros_like(rho0), torch.zeros_like(rho0), device=device
    )
    f = equilibrium3d(
        rho0,
    ...
    window_f = []
    print(
        f"Re={re:.0f} r={radius:.0f} ref Cd={cd_ref(re):.4f}  convergence (BFL Cd per 500-step window):"
    )
    for step in range(1, n_steps + 1):
        ...
        if far_field:
            ...
            f = apply_zou_he_channel_boundaries_3d(
                f, u_in=u_in, wall_mask=wall_mask, obstacle_mask=torch.zeros_like(solid)
            )
        f_pre = f.clone()
        for d in range(1, 19):
    ...
    for label, L in (
        ("BFL momentum-exchange", f_force),
        ("CV momentum+pressure", cv_force),
        ("wake deficit", wake_force),
    ):
        cd = avg(L) / dyn_p
