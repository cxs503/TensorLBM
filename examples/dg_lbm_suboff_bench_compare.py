...
def run_one(use_real_dg, cfg_dict, out, label):
    ...
    m = json.loads(
        (
            pathlib.Path(out) / "dg_lbm_suboff" / cfg_dict["run_name"] / "run_metadata.json"
        ).read_text()
    )
...
def main():
    ...
    common = dict(
        ...
        re=100.0,
        hull_length=80.0,
        hull_type=SuboffHullType.BARE_HULL.value,
        dg_band=4.0,
        n_steps=250,
        output_interval=50,
        output_root=out,
    ...
    area, _ = wetted_area(
        "bare_hull", common["nx"], common["ny"], common["nz"], common["hull_length"]
    )
    u_in = common["u_in"]
    re = common["re"]
    ...
    print(
        f"reference: Cf_Blasius = {cf_blasius:.4f}  form-factor(1+k) = {ff:.2f}  Cd_ref = {cd_ref:.4f}\n"
    )
