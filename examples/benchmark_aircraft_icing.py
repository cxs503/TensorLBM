...
def main() -> None:
    ...
    p.add_argument(
        "--no-prefill", action="store_true", help="disable the t=0 steady-cloud inventory seeding"
    )
    p.add_argument(
        "--ab-clean",
        action="store_true",
    ...
    print(
        f"  rho_rime = {cfg.rho_rime_eff:.0f} kg/m^3 "
    ...
    if "max_impact_x_frac" in m:
        print(
            f"    impingement    : max impact x = LE + {m['max_impact_x_frac'] * 100:.1f} % chord"
        )
    print("-" * 72)
    b = r["beta"]
    if len(b["beta"]):
        imax = b["beta"].argmax()
        print(f"  BETA: max={b['beta'].max():.3f} at s/c={b['s_over_c'][imax] * 100:+.2f} % chord")
    e = r.get("eulerian")
    if e is not None and len(e["beta"]["beta"]):
        be = e["beta"]
        imax = be["beta"].argmax()
        print(
            f"  BETA-E (eulerian): max={be['beta'].max():.3f} at s/c="
            f"{be['s_over_c'][imax] * 100:+.2f} % chord, "
            f"capture height={float(e['beta_grid'].sum()):.2f} cells"
        )
        if len(b["beta"]):
            print(f"  BETA   (lagrangian): capture height={float(r['beta_grid'].sum()):.2f} cells")
    if r["cd0"] is not None and r["cd_end"] is not None:
        print(
            f"  AERO: cd {r['cd0']:.5f} -> {r['cd_end']:.5f} "
            f"({r['cd_drift_pct']:+.1f} %), cl {r['cl0']:.5f} -> {r['cl_end']:.5f}"
        )
        print(
            f"        reference cl (thin airfoil, AoA={args.aoa:.0f} deg): "
    ...
    if args.ab_clean and r["cd0"] is not None:
        ...
        print(f"  A/B : cd_clean(end)={twin['cd_end']:.5f} cl_clean(end)={twin['cl_end']:.5f}")
        print(
            f"        ice feedback on cd: {dcd:+.1f} %  "
