...
def run(
    ...
    solid, stats = build_hull_mask(hull_type, nx, ny, nz, cx=cx, cy=cy, cz_keel=cz, device=device)
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 1000 == 0 or step == n_steps:
            ...
            print(
                f"  step {step:4d}: Ct_fric={cf:.5f} Ct_pres={cp:.5f} Ct_tot={cf + cp:.5f} (ref {ct_ref:.5f})"
            )
    cf = (sum(fric) / max(len(fric), 1)) / dyn_p_S
    cp = (sum(pres) / max(len(pres), 1)) / dyn_p_S
    ct = cf + cp
