...
def run_moving_sphere(nx=128, ny=64, nz=64, n_steps=3000, device="sdaa:0", stationary=False):
    ...
    cx_s = 32.0
    cy_s = ny // 2
    cz_s = nz // 2  # sphere center (float for smooth motion)
    ...
    for step in range(1, n_steps + 1):
        ...
        if bool(new_fluid.any()):
            ...
            for sz_s, sy_s, sx_s in [
                ...
                prev_boundary |= prev_fluid & torch.roll(
                    prev_solid, shifts=(sz_s, sy_s, sx_s), dims=(0, 1, 2)
                )
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            print(
                f"step {step:4d}: cx={cx_int} Fx={force_x:.2f} Cd={cd:.3f} "
