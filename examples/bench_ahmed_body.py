...
def build_ahmed_body(nx, ny, nz, slant_deg=25.0, device="cpu"):
    ...
    in_body = (
        (xx >= x0) & (xx < x1) & (yy >= cy - W / 2) & (yy < cy + W / 2) & (zz >= cz) & (zz < cz + H)
    )
    # Slant back (angled)
...
def run_ahmed(slant_deg=25.0, device="sdaa:0", n_steps=1500, warmup=400):
    ...
    for step in range(1, n_steps + 1):
        ...
        if step % 300 == 0 or step == n_steps:
            ...
            print(
                f"  step {step}: Cf={cf:.4f} Cp={cp:.4f} Cd={cf + cp:.4f} (ref={cd_ref})",
                flush=True,
            )
    dt = time.time() - t0
    cf = sum(fric) / max(len(fric), 1) / dyn_p_S
